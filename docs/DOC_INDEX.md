...
| **Messaging Archive** | `platform_data/knowledge/Messages/inbox/` | Legacy message archive (read-only). |
| **The Guard** | [tests/README.md](./tests/README.md) | You are writing tests (which you should be). |
| **The Memory** | [platform_data/knowledge/README.md](./platform_data/knowledge/README.md) | You need to access DB, RAG, or Messages. |
...
- **Tasks:** `platform_data/knowledge/LocalDB/tasks.db`

### 3. Communication
- **Primary:** `scripts/ybis.py message ...` (MCP + SQLite)
- **Archive:** `platform_data/knowledge/Messages/inbox/` (read-only)
