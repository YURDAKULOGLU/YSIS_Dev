...
[Skip to content](#context-precision)
...
Where \(K\) is the total number of chunks in `retrieved_contexts` and \(v\_k \in \{0, 1\}\) is the relevance indicator at rank \(k\).
...
The metric computes precision using retrieved\_context\_ids and reference\_context\_ids, with values ranging between 0 and 1. Higher values indicate better performance. It works with both string and integer IDs.
...
In this example, out of the 4 retrieved context IDs, only 2 ("doc\_1" and "doc\_4") are found in the reference context IDs, resulting in a precision score of 0.5 or 50%.
