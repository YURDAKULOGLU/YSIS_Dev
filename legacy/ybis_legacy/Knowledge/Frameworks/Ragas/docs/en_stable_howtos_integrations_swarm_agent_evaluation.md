...
[Skip to content](#installing-ragas-and-other-dependencies)
...
4. If the reason is valid, the agent moves on to check eligibility.
5. If the reason is invalid, the agent responds empathetically and explains the return policy to the customer.
...
To convert a list of Swarm messages into a format suitable for Ragas evaluation, Ragas provides the function [convert\_to\_ragas\_messages][ragas.integrations.swarm.convert\_to\_ragas\_messages], which can be used to transform LangChain messages into the format expected by Ragas.
