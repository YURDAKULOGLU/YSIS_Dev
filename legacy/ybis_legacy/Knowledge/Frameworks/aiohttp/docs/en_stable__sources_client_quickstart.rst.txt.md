...
To make several requests to the same site more simple, the parameter ``base\_url``
...
200
'[{"created\_at":"2015-06-12T14:06:22Z","public":true,"actor":{...
...
::
b'[{"created\_at":"2015-06-12T14:06:22Z","public":true,"actor":{...
...
``serializer``. :class:`ClientSession` accepts ``json\_serialize``
parameter::
import ujson
async with aiohttp.ClientSession(
json\_serialize=ujson.dumps) as session:
...
async for chunk in resp.content.iter\_chunked(chunk\_size):
fd.write(chunk)
...
You can set the ``filename`` and ``content\_type`` explicitly::
url = 'http://httpbin.org/post'
data = aiohttp.FormData()
data.add\_field('file',
open('report.xls', 'rb'),
filename='report.xls',
content\_type='application/vnd.ms-excel')
...
async def file\_sender(file\_name=None):
async with aiofiles.open(file\_name, 'rb') as f:
chunk = await f.read(64\*1024)
while chunk:
yield chunk
chunk = await f.read(64\*1024)
# Then you can use file\_sender as a data provider:
async with session.post('http://httpbin.org/post',
data=file\_sender(file\_name='huge\_file')) as resp:
print(await resp.text())
...
``async\_generator`` library as workaround.
.. deprecated:: 3.1
...
You have to use the :meth:`aiohttp.ClientSession.ws\_connect` coroutine
...
methods::
async with session.ws\_connect('http://example.org/ws') as ws:
async for msg in ws:
...
``await ws.send\_str('data')`` for example).
...
the default ``sock\_connect`` timeout is 30 seconds.
...
sock\_connect=None, sock\_read=None, ceil\_threshold=5)
.. note::
...
value of 5 seconds can be configured using the ``ceil\_threshold`` parameter.