...
When chunked transfer encoding is used, end\_of\_HTTP\_chunk is a :class:`bool`
...
.. attribute:: StreamReader.total\_raw\_bytes
...
.. method:: StreamReader.iter\_chunked(n)
:async:
...
`asyncstdlib.itertools `\_
module::
chunks = batched(chain.from\_iterable(response.content.iter\_chunked(n)), n)
async for data in chunks:
print(data)
.. method:: StreamReader.iter\_any()
:async:
Iterates over data chunks in order of intaking them into the stream::
async for data in response.content.iter\_any():
print(data)
.. method:: StreamReader.iter\_chunks()
:async:
Iterates over data chunks as received from the server::
async for data, \_ in response.content.iter\_chunks():
print(data)
...
buffer = b""
async for data, end\_of\_http\_chunk in response.content.iter\_chunks():
buffer += data
if end\_of\_http\_chunk:
print(buffer)
buffer = b""
Helpers
-------
...
.. seealso::
:meth:`StreamReader.at\_eof`
.. method:: StreamReader.at\_eof()
Return ``True`` if the buffer is empty and EOF was reached.
.. method:: StreamReader.read\_nowait(n=None)
...
buffer.
:return bytes: the given data
.. method:: StreamReader.unread\_data(data)
