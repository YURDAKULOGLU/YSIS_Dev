Change history — Celery 5.6.2 documentation

### Navigation

* [index](../genindex.html "General Index")
* [modules](../py-modindex.html "Python Module Index") |
* [next](whatsnew-3.0.html "What’s new in Celery 3.0 (Chiastic Slide)") |
* [previous](whatsnew-3.1.html "What’s new in Celery 3.1 (Cipater)") |
...
[go here](https://docs.celeryq.dev/en/main/history/changelog-3.1.html).

# Change history[¶](#change-history "Link to this heading")

This document contains change notes for bugfix releases in the 3.1.x series
(Cipater), please see [What’s new in Celery 3.1 (Cipater)](whatsnew-3.1.html#whatsnew-3-1) for an overview of what’s
new in Celery 3.1.

## 3.1.26[¶](#version-3-1-26 "Link to this heading")
...
## 3.1.25[¶](#version-3-1-25 "Link to this heading")
...
* **Requirements**
  ...
  > + Now depends on [Kombu 3.0.37](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-37 "(in Kombu v5.6)")
* Fixed problem with chords in group introduced in 3.1.24 (Issue #3504).

## 3.1.24[¶](#version-3-1-24 "Link to this heading")
...
* **Requirements**
  ...
  > + Now depends on [Kombu 3.0.36](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-36 "(in Kombu v5.6)").
* Now supports Task protocol 2 from the future 4.0 release.

  > Workers running 3.1.24 are now able to process messages
  > sent using the [new task message protocol](https://docs.celeryq.dev/en/master/internals/protocol.html#version-2) to be introduced
  > in Celery 4.0.
  >
...
* `Task.send_events` can now be set to disable sending of events
  ...
  >     return x + y
  > ```
* **Utils**: Fixed compatibility with recent <https://pypi.org/project/psutil/> versions
  (Issue #3262).
...
* **Tasks**: GroupResult now defines \_\_bool\_\_ and \_\_nonzero\_\_.
  ...
  > result list are not properly tupled with the as\_tuple() method when it is
  > a parent result. This is due to the as\_tuple() method performing a logical
...
## 3.1.23[¶](#version-3-1-23 "Link to this heading")
...
* **Requirements**
  ...
  > + Now depends on [Kombu 3.0.34](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-34 "(in Kombu v5.6)").
  > + Now depends on `billiard` 3.3.0.23.
* **Prefork pool**: Fixes 100% CPU loop on Linux *epoll*
  (Issue #1845).
...
* **Programs**: **celery shell --ipython** now compatible with newer
  <https://pypi.org/project/IPython/> versions.
...
* **Utils**: The `.discard(item)` method of
  [`LimitedSet`](../internals/reference/celery.utils.collections.html#celery.utils.collections.LimitedSet "celery.utils.collections.LimitedSet") didn’t actually remove the item
  (Issue #3087).
...
## 3.1.20[¶](#version-3-1-20 "Link to this heading")
...
* **Requirements**
  ...
  > + Now depends on [Kombu 3.0.33](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-33 "(in Kombu v5.6)").
  > + Now depends on `billiard` 3.3.0.22.
  >
  >   > Includes binary wheels for Microsoft Windows x86 and x86\_64!
...
* **Worker**: Bootsteps can now hook into `on_node_join`/`leave`/`lost`.
  ...
  > See [Attributes](../userguide/extending.html#extending-consumer-attributes) for an example.
...
* **Results**: Redis `new_join` didn’t properly call task errbacks on chord
  error (Issue #2796).
* **Results**: Restores Redis compatibility with Python <https://pypi.org/project/redis/> < 2.10.0
  (Issue #2903).
* **Results**: Fixed rare issue with chord error handling (Issue #2409).
* **Tasks**: Using queue-name values in `CELERY_ROUTES` now works
  again (Issue #2987).
...
* **Beat**: Now syncs the schedule even if the schedule is empty.
  ...
  > Fix contributed by Colin McIntosh.
* **Supervisord**: Set higher process priority in the <https://pypi.org/project/supervisord/>
  :   example.

      Contributed by George Tantiras.
* **Documentation**: Includes improvements by:

  > [@Bryson](https://github.com/Bryson/)
  > Caleb Mingle
  > Christopher Martin
  > Dieter Adriaenssens
  > Jason Veatch
  ...
  > Kirill Pavlov
  > Marco Buttu
  > [@Mayflower](https://github.com/Mayflower/)
  > Mher Movsisyan
  > Michael Floering
  ...
  > Ryan Luckie
  > Steven Parker
  > [@squfrans](https://github.com/squfrans/)
  > Tadej Janež
  > TakesxiSximada
  > Tom S

## 3.1.19[¶](#version-3-1-19 "Link to this heading")
...
* **Requirements**
  ...
  > + Now depends on [Kombu 3.0.29](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-29 "(in Kombu v5.6)").
  > + Now depends on `billiard` 3.3.0.21.
* **Results**: Fixed MongoDB result backend URL parsing problem
  (Issue celery/kombu#375).
* **Worker**: Task request now properly sets `priority` in delivery\_info.
...
* **Documentation**: Many improvements and typos fixed.
  ...
  > > Carlos Garcia-Dubus
  > > D. Yu
  > > [@jerry](https://github.com/jerry/)
  > > Jocelyn Delalande
  > > Josh Kupershmidt
  > > Juan Rossi
  > > [@kanemra](https://github.com/kanemra/)
  > > Paul Pearce
  > > Pavel Savchenko
  > > Sean Wang
  > > Seungha Kim
...
## 3.1.18[¶](#version-3-1-18 "Link to this heading")
...
* **Requirements**
  ...
  > + Now depends on [Kombu 3.0.25](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-25 "(in Kombu v5.6)").
  > + Now depends on `billiard` 3.3.0.20.
...
* **Django**: Fixed a [`TypeError`](https://docs.python.org/dev/library/exceptions.html#TypeError "(in Python v3.15)") sometimes occurring in logging
  when validating models.

  > Fix contributed by Alexander.
* **Commands**: Worker now supports new
  [`--executable`](../reference/cli.html#cmdoption-celery-worker-executable) argument that can
  be used with [`celery worker --detach`](../reference/cli.html#cmdoption-celery-worker-D).
...
* **Worker**: Fixed rare crash occurring with
  [`--autoscale`](../reference/cli.html#cmdoption-celery-worker-autoscale) enabled (Issue #2411).
...
## 3.1.17[¶](#version-3-1-17 "Link to this heading")
...
Don’t enable the CELERYD\_FORCE\_EXECV setting!
...
* **Requirements**
  ...
  > + Now depends on [Kombu 3.0.24](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-24 "(in Kombu v5.6)").
  >
...
* **Task**: The timing of ETA/countdown tasks were off after the example `LocalTimezone`
  ...
  (Issue #2306).
* **Task**: Raising [`Ignore`](../reference/celery.exceptions.html#celery.exceptions.Ignore "celery.exceptions.Ignore") no longer sends
...
* **Results**: `.join_native()` would accidentally treat the `STARTED`
  ...
  > when using chords with the `CELERY_TRACK_STARTED` setting
  > enabled.
...
* **Task**: Fixed problem with app not being properly propagated to
  ...
  > Fix contributed by [@kristaps](https://github.com/kristaps/).
...
* **Task**: Exception info wasn’t properly set for tasks raising
  [`Reject`](../reference/celery.exceptions.html#celery.exceptions.Reject "celery.exceptions.Reject") (Issue #2043).
...
* **celery.contrib.rdb**: Fixed problems with `rdb.set_trace` calling stop
  ...
  > Fix contributed by [@llllllllll](https://github.com/llllllllll/).
...
## 3.1.16[¶](#version-3-1-16 "Link to this heading")
...
* **Worker**: 3.1.15 broke [`-Ofair`](../reference/cli.html#cmdoption-celery-worker-O)
  behavior (Issue #2286).
...
* **Django**: Adds support for Django 1.7 class names in INSTALLED\_APPS
...
## 3.1.15[¶](#version-3-1-15 "Link to this heading")
...
* **Results**: `result.get()` was misbehaving by calling
  `backend.get_task_meta` in a [`finally`](https://docs.python.org/dev/reference/compound_stmts.html#finally "(in Python v3.15)") call leading to
  AMQP result backend queues not being properly cleaned up (Issue #2245).

## 3.1.14[¶](#version-3-1-14 "Link to this heading")
...
* **Requirements**
  ...
  > + Now depends on [Kombu 3.0.22](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-22 "(in Kombu v5.6)").
...
* **Programs**: `--umask` argument can now be
  specified in both octal (if starting with 0) or decimal.

## 3.1.13[¶](#version-3-1-13 "Link to this heading")

### Security Fixes[¶](#security-fixes "Link to this heading")

* [Security: [CELERYSA-0002](https://github.com/celery/celery/tree/master/docs/sec/CELERYSA-0002.txt)] Insecure default umask.
...
### News[¶](#news "Link to this heading")

* **Requirements**

  > + Now depends on [Kombu 3.0.21](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-21 "(in Kombu v5.6)").
  > + Now depends on `billiard` 3.3.0.18.
* **App**: `backend` argument now also sets the `CELERY_RESULT_BACKEND`
  setting.
...
* Documentation fixes
  ...
  > Contributed by Yuval Greenfield, Lucas Wiman, [@nicholsonjf](https://github.com/nicholsonjf/).
* **Worker**: Removed an outdated assert statement that could lead to errors
  being masked (Issue #2086).

## 3.1.12[¶](#version-3-1-12 "Link to this heading")
...
* **Requirements**
  ...
  > Now depends on [Kombu 3.0.19](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-19 "(in Kombu v5.6)").
...
* **Programs**: **celery worker --detach** no longer closes open file
  descriptors when `C_FAKEFORK` is used so that the workers output
  can be seen.
...
* **Programs**: The **celery multi stopwait** command is now documented
  in usage.
...
* **Programs**: **celery worker --detach** didn’t forward working
  directory option (Issue #2003).
* **Programs**: **celery inspect registered** no longer includes
...
* **Init-scripts**: The CentOS init-scripts didn’t quote
  ...
  > Fix contributed by [@ffeast](https://github.com/ffeast/).

## 3.1.11[¶](#version-3-1-11 "Link to this heading")
...
* **Requirements**:
  ...
  > + Now depends on [Kombu 3.0.15](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-15 "(in Kombu v5.6)").
...
* **Tasks**: The `CELERY_DEFAULT_DELIVERY_MODE` setting was being
  ignored (Issue #1953).
* **Worker**: New [`celery worker --heartbeat-interval`](../reference/cli.html#cmdoption-celery-worker-heartbeat-interval) can be used
...
## 3.1.10[¶](#version-3-1-10 "Link to this heading")
...
* **Requirements**:
  ...
  > + Now depends on [Kombu 3.0.14](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-14 "(in Kombu v5.6)").
* **Results**:
...
* **Redis:** Important note about events (Issue #1882).
  ...
  > nodes.
  >
  > See [Caveats](../getting-started/backends-and-brokers/redis.html#redis-caveats).
  >
  > This will be the default in Celery 3.2.
* **Results**: The [`app.AsyncResult`](../reference/celery.result.html#celery.result.AsyncResult "celery.result.AsyncResult") object now keeps a local cache
  of the final state of the task.

  > This means that the global result cache can finally be disabled,
  > and you can do so by setting `CELERY_MAX_CACHED_RESULTS` to
  > `-1`. The lifetime of the cache will then be bound to the
...
* **Worker**: Ability to use one log file per child process when using the
  ...
  > for the log file name. See [Prefork pool process index](../userguide/workers.html#worker-files-process-index).
...
* **Task**: A regression caused the `queue` argument to `Task.retry` to be
  ignored (Issue #1892).
* **App**: Fixed error message for [`config_from_envvar()`](../reference/celery.html#celery.Celery.config_from_envvar "celery.Celery.config_from_envvar").
...
* **Task**: `Task.apply` now properly sets `request.headers`
  (Issue #1874).
* **Worker**: Fixed [`UnicodeEncodeError`](https://docs.python.org/dev/library/exceptions.html#UnicodeEncodeError "(in Python v3.15)") occurring when worker is started
  by <https://pypi.org/project/supervisor/>.
...
* **Commands**: **celery inspect memdump** no longer crashes
  if the `psutil` module isn’t installed (Issue #1914).
...
## 3.1.9[¶](#version-3-1-9 "Link to this heading")
...
* **Requirements**:
  ...
  > + Now depends on [Kombu 3.0.12](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-12 "(in Kombu v5.6)").
...
* **New Sphinx extension**: for autodoc documentation of tasks:
  `celery.contrib.spinx` (Issue #1833).
...
* **Generic init-scripts**: Fixed compatibility with the minimal
  **dash** shell (Issue #1815).
* **Commands**: The **celery amqp basic.publish** command wasn’t
  working properly.
...
* **Commands**: Better error message for missing arguments to preload
  options (Issue #1860).
* **Commands**: **celery -h** didn’t work because of a bug in the
...
* **Beat**: Fixed problem with beat hanging after the first schedule
  ...
  > Fix contributed by Roger Hu.
* **Signals**: The header argument to [`before_task_publish`](../userguide/signals.html#std-signal-before_task_publish) is now
...
## 3.1.8[¶](#version-3-1-8 "Link to this heading")
...
* **Requirements**:
  ...
  > + Now depends on [Kombu 3.0.10](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-10 "(in Kombu v5.6)").
  > + Now depends on [billiard 3.3.0.14](https://github.com/celery/billiard/blob/master/CHANGES.txt).
...
* **Results:** `ResultSet.iterate` is now pending deprecation.
  ...
  > instead.
* **Worker**|eventlet/gevent: A regression caused `Control-c` to be
...
* **Commands**: The **celery purge** command now warns that the operation
  will delete all tasks and prompts the user for confirmation.

  > A new [`-f`](../reference/cli.html#cmdoption-celery-purge-f) was added that can be used to disable
  > interactive mode.
* **Task**: `.retry()` didn’t raise the value provided in the `exc` argument
  when called outside of an error context (*Issue #1755*).
* **Commands:** The **celery multi** command didn’t forward command
...
* **Commands:** The output of the event dump utility
  (**celery events -d**) can now be piped into other commands.
...
* **Commands:** The [`--no-color`](../reference/cli.html#cmdoption-celery-C) argument was
...
* **Documentation:** <https://pypi.org/project/supervisor/> examples contained an
  extraneous ‘-’ in a [`--logfile`](../reference/cli.html#cmdoption-celery-worker-f) argument
  example.

  > Fix contributed by Mohammad Almeer.

## 3.1.7[¶](#version-3-1-7 "Link to this heading")
...
### Init-script security improvements[¶](#init-script-security-improvements "Link to this heading")
...
#### AMQP result backend[¶](#amqp-result-backend "Link to this heading")
...
#### Synchronous subtasks[¶](#synchronous-subtasks "Link to this heading")

Tasks waiting for the result of a subtask will now emit
a [`RuntimeWarning`](https://docs.python.org/dev/library/exceptions.html#RuntimeWarning "(in Python v3.15)") warning when using the prefork pool,
...
deadlock when using the prefork pool (see also [Avoid launching synchronous subtasks](../userguide/tasks.html#task-synchronous-subtasks)).
...
behavior with the worker [-Ofair option](../userguide/optimizing.html#optimizing-prefetch-limit).

### Fixes[¶](#fixes "Link to this heading")

* Now depends on [Kombu 3.0.8](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-8 "(in Kombu v5.6)").
* Now depends on `billiard` 3.3.0.13
* Events: Fixed compatibility with non-standard json libraries
  that sends float as [`decimal.Decimal`](https://docs.python.org/dev/library/decimal.html#decimal.Decimal "(in Python v3.15)") (Issue #1731)
...
* Worker: Errors occurring while attempting to serialize the result of a
  ...
  `kombu.exceptions.EncodingError` error.

  > Fix contributed by Ionel Cristian Mărieș.
* Worker with [`-B`](../reference/cli.html#cmdoption-celery-worker-B) argument didn’t properly
...
* Worker: The `%n` and `%h` formats are now also supported by the
  ...
  [`--statedb`](../reference/cli.html#cmdoption-celery-worker-S) arguments.
...
* Redis/Cache result backends: Will now timeout if keys evicted while trying
  to join a chord.
* The fallback unlock chord task now raises `Retry` so that the
...
## 3.1.6[¶](#version-3-1-6 "Link to this heading")
...
* Now depends on [Kombu 3.0.7](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-7 "(in Kombu v5.6)").
...
## 3.1.5[¶](#version-3-1-5 "Link to this heading")
...
* Now depends on [Kombu 3.0.6](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-6 "(in Kombu v5.6)").
...
* App: `autodiscover_tasks` is now lazy.
  ...
  > prematurely.
* Fixed regression for [`--app`](../reference/cli.html#cmdoption-celery-A) argument
  experienced by some users (Issue #1653).
* Worker: Now respects the [`--uid`](../reference/cli.html#cmdoption-celery-worker-uid) and
  [`--gid`](../reference/cli.html#cmdoption-celery-worker-gid) arguments even if
  [`--detach`](../reference/cli.html#cmdoption-celery-worker-D) isn’t enabled.
* Beat: Now respects the [`--uid`](../reference/cli.html#cmdoption-celery-beat-uid) and
  [`--gid`](../reference/cli.html#cmdoption-celery-beat-gid) arguments even if
  [`--detach`](../reference/cli.html#cmdoption-celery-beat-detach) isn’t enabled.
* Python 3: Fixed unorderable error occurring with the worker
  [`-B`](../reference/cli.html#cmdoption-celery-worker-B) argument enabled.
...
* The Django daemonization tutorial was changed.
  ...
  > in `/etc/default/celeryd` when the new project layout is used.
...
* [`task_postrun`](../userguide/signals.html#std-signal-task_postrun) is now sent within a [`finally`](https://docs.python.org/dev/reference/compound_stmts.html#finally "(in Python v3.15)") block,
  to make sure the signal is always sent.
* Beat: Fixed syntax error in string formatting.

  > Contributed by [@nadad](https://github.com/nadad/).
* Fixed typos in the documentation.

  > Fixes contributed by Loic Bistuer, [@sunfinite](https://github.com/sunfinite/).
* Nested chains now works properly when constructed using the
  `chain` type instead of the `|` operator (Issue #1656).

## 3.1.4[¶](#version-3-1-4 "Link to this heading")
...
* Now depends on [Kombu 3.0.5](https://docs.celeryq.dev/projects/kombu/en/main/changelog.html#version-3-0-5 "(in Kombu v5.6)").
...
* The [`--app`](../reference/cli.html#cmdoption-celery-A) argument could end up using a module
...
## 3.1.3[¶](#version-3-1-3 "Link to this heading")
...
* Fixed compatibility problem with Python 2.7.0 - 2.7.5 (Issue #1637)
  ...
  > in Python 2.7.6.
* Worker: [`-B`](../reference/cli.html#cmdoption-celery-worker-B) argument accidentally closed
  files used for logging.
* Task decorated tasks now keep their docstring (Issue #1636)

## 3.1.2[¶](#version-3-1-2 "Link to this heading")
...
* Now depends on `billiard` 3.3.0.6
...
* Django: Fixed `ImproperlyConfigured` error raised
  ...
  > Fix contributed by [@j0hnsmith](https://github.com/j0hnsmith/).
...
## 3.1.1[¶](#version-3-1-1 "Link to this heading")
...
* Now depends on `billiard` 3.3.0.4.
...
* Django: Tutorial updated with a solution that sets a default
  [`DJANGO_SETTINGS_MODULE`](http://docs.djangoproject.com/en/dev/topics/settings/#envvar-DJANGO_SETTINGS_MODULE "(in Django v6.1)") so that it doesn’t have to be typed
  in with the **celery** command.

  > Also fixed typos in the tutorial, and added the settings
  > required to use the Django database backend.
  >
  > Thanks to Chris Ward, [@orarbel](https://github.com/orarbel/).
...
## 3.1.0[¶](#version-3-1-0 "Link to this heading")
...
See [What’s new in Celery 3.1 (Cipater)](whatsnew-3.1.html#whatsnew-3-1).
...
[What’s new in Celery 3.1 (Cipater)](whatsnew-3.1.html "previous chapter")

#### Next topic

[What’s new in Celery 3.0 (Chiastic Slide)](whatsnew-3.0.html "next chapter")

### This Page

* [Show Source](../_sources/history/changelog-3.1.rst.txt)
