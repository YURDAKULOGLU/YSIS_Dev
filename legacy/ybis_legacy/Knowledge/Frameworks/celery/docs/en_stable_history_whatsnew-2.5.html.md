What’s new in Celery 2.5 — Celery 5.6.2 documentation

### Navigation

* [index](../genindex.html "General Index")
* [modules](../py-modindex.html "Python Module Index") |
* [next](changelog-2.5.html "Change history for Celery 2.5") |
* [previous](changelog-3.0.html "Change history for Celery 3.0") |
...
[go here](https://docs.celeryq.dev/en/main/history/whatsnew-2.5.html).

# What’s new in Celery 2.5[¶](#what-s-new-in-celery-2-5 "Link to this heading")
...
to [django-celery 2.5](https://pypi.org/project/django-celery/).

This version is officially supported on CPython 2.5, 2.6, 2.7, 3.2 and 3.3,
as well as PyPy and Jython.

## [Important Notes](#id1)[¶](#important-notes "Link to this heading")

### [Broker connection pool now enabled by default](#id2)[¶](#broker-connection-pool-now-enabled-by-default "Link to this heading")
...
`CELERY_TASK_PUBLISH_RETRY` and
`CELERY_TASK_PUBLISH_RETRY_POLICY`.

### [Rabbit Result Backend: Exchange is no longer *auto delete*](#id3)[¶](#rabbit-result-backend-exchange-is-no-longer-auto-delete "Link to this heading")

The exchange used for results in the Rabbit (AMQP) result backend
used to have the *auto\_delete* flag set, which could result in a
race condition leading to an annoying warning.

For RabbitMQ users

Old exchanges created with the *auto\_delete* flag enabled has
to be removed.

The **camqadm** command can be used to delete the
previous exchange:
...
### [Solution for hanging workers (but must be manually enabled)](#id4)[¶](#solution-for-hanging-workers-but-must-be-manually-enabled "Link to this heading")

The CELERYD\_FORCE\_EXECV setting has been added to solve
...
* See [Python Issue 6721](http://bugs.python.org/issue6721#msg140215) to read more about this issue, and why
...
## [Optimization](#id5)[¶](#optimization "Link to this heading")
...
* The code path used when the worker executes a task has been heavily
  ...
  when using the <https://pypi.org/project/pylibrabbitmq/> transport, where it previously
...
## [Deprecation Time-line Changes](#id6)[¶](#deprecation-time-line-changes "Link to this heading")

### [Removals](#id7)[¶](#removals "Link to this heading")

* The old `TaskSet` signature of `(task_name, list_of_tasks)`
...
* It’s no longer possible to import `subtask` and `TaskSet`
  from `celery.task.base`, please import them from `celery.task`
  instead (originally scheduled for removal in 2.4).

### [Deprecated modules](#id8)[¶](#deprecated-modules "Link to this heading")

* The `celery.decorators` module has changed status
...
## [News](#id9)[¶](#news "Link to this heading")

### [Timezone support](#id10)[¶](#timezone-support "Link to this heading")
...
setting. Installing the <https://pypi.org/project/pytz/> library is recommended when
...
<https://pypi.org/project/django-celery/> will use the local timezone as specified by the
`TIME_ZONE` setting, it will also honor the new [USE\_TZ](https://docs.djangoproject.com/en/dev/topics/i18n/timezones/) setting
introduced in Django 1.4.

### [New security serializer using cryptographic signing](#id11)[¶](#new-security-serializer-using-cryptographic-signing "Link to this heading")
...
configuration to work (see [Security](../userguide/configuration.html#conf-security)).

See also

[Security](../userguide/security.html#guide-security)

Contributed by Mher Movsisyan.

### [New `CELERY_ANNOTATIONS` setting](#id12)[¶](#new-celery-annotations-setting "Link to this heading")
...
### [`current` provides the currently executing task](#id13)[¶](#current-provides-the-currently-executing-task "Link to this heading")

The new `celery.task.current` proxy will always give the currently
executing task.
...
### [In Other News](#id14)[¶](#in-other-news "Link to this heading")
...
* Now limits the number of frames in a traceback so that `celeryd` doesn’t
  ...
  > [`sys.getrecursionlimit()`](https://docs.python.org/dev/library/sys.html#sys.getrecursionlimit "(in Python v3.15)") and [`sys.setrecursionlimit()`](https://docs.python.org/dev/library/sys.html#sys.setrecursionlimit "(in Python v3.15)").
...
* `task.retry()` now re-raises the original exception keeping
  ...
  > Suggested by [@ojii](https://github.com/ojii/).
...
* `celeryctl`: Added `shell` command.
  ...
  > The shell will have the current\_app (`celery`) and all tasks
...
* Routers can now override the `exchange` and `routing_key` used
  ...
  > but you can now have a router return exchange and routing\_key keys
  > to set them.
  >
...
* Redis result backend: Adds support for a `max_connections` parameter.
  ...
  > `CELERY_REDIS_MAX_CONNECTIONS` setting,
...
* There’s a new [Security](../userguide/security.html#guide-security) guide in the documentation.
...
* Can now configure `CELERYCTL` environment variable
  in `/etc/default/celeryd`.

  > While not necessary for operation, **celeryctl** is used for the
  > `celeryd status` command, and the path to **celeryctl** must be
...
## [Fixes](#id15)[¶](#fixes "Link to this heading")
...
* Redis result backend: Now uses `SETEX` command to set result key,
  ...
  > Suggested by [@yaniv-aknin](https://github.com/yaniv-aknin/).
* `celeryd`: Fixed a problem where shutdown hanged when `Control-c`
...
* Cassandra backend: No longer uses `pycassa.connect()` which is
  deprecated since <https://pypi.org/project/pycassa/> 1.4.
...
* `send_task` now emits a warning when used in combination with
  `CELERY_ALWAYS_EAGER` (Issue #581).

  > Contributed by Mher Movsisyan.
* `apply_async` now forwards the original keyword arguments to `apply`
  when `CELERY_ALWAYS_EAGER` is enabled.
...
* `celeryctl`: The [`--expires`](../reference/cli.html#cmdoption-celery-call-expires) and
  [`--eta`](../reference/cli.html#cmdoption-celery-call-eta) arguments to the apply command
...
[Change history for Celery 3.0](changelog-3.0.html "previous chapter")

#### Next topic

[Change history for Celery 2.5](changelog-2.5.html "next chapter")

### This Page

* [Show Source](../_sources/history/whatsnew-2.5.rst.txt)
