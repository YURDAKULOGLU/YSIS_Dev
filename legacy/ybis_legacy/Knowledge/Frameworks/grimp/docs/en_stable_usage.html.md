Usage — Grimp 3.14 documentation
...
taken in part from [the official Python docs](https://docs.python.org/3/tutorial/modules.html):
...
* **Graph**: A graph [in the mathematical sense](https://en.wikipedia.org/wiki/Graph_(discrete_mathematics)) of a collection of items with relationships between them. Grimp’s
...
## Building the graph[](#building-the-graph "Link to this heading")
...
grimp.build\_graph(*package\_name*, *\*additional\_package\_names*, *include\_external\_packages=False*, *exclude\_type\_checking\_imports=False*)[](#grimp.build_graph "Link to this definition")
:   Build and return an ImportGraph for the supplied package or packages.

    Parameters:
    :   * **package\_name** (*str*) – The name of an importable package, for example `'mypackage'`. For regular packages, this
          must be the top level package (i.e. one with no dots in its name). In the special case of
          [namespace packages](https://docs.python.org/3/glossary.html#term-namespace-package), the name of the *portion* may be supplied instead, for example `'mynamespace.foo'`.
          If the portion is supplied, its ancestor packages will not be included in the graph.
        * **additional\_package\_names** (*tuple**[**str**,* *...**]*) – Tuple of any additional package names. These can be
          supplied as positional arguments, as in the example above.
        * **include\_external\_packages** (*bool**,* *optional*) –

          Whether to include external packages in the import graph. If this is `True`,
          any other top level packages (including packages in the standard library) that are imported by this package will
          be included in the graph as squashed modules (see [Terminology](#terminology) above).

          The behaviour is more complex if one of the specified packages is a [namespace portion](https://docs.python.org/3/glossary.html#term-portion).
        ...
        * **exclude\_type\_checking\_imports** (*bool**,* *optional*) – Whether to exclude imports made in type checking guards. If this is `True`,
          any import made under an `if TYPE_CHECKING:` statement will not be added to the graph.
          See the [typing module documentation](https://docs.python.org/3/library/typing.html#typing.TYPE_CHECKING) for reference. (The type checking guard is detected purely by looking for
        ...
        * **cache\_dir** (*str**,* *optional*) – The directory to use for caching the graph. Defaults to `.grimp_cache`. To disable caching,
          pass `None`. See [Caching](caching.html).
...
## Methods for analysing the module tree[](#methods-for-analysing-the-module-tree "Link to this heading")

ImportGraph.modules[](#ImportGraph.modules "Link to this definition")
...
ImportGraph.find\_children(*module*)[](#ImportGraph.find_children "Link to this definition")
:   Return all the immediate children of the module, i.e. the modules that have a dotted module name that is one
    level below.

    > param str module:
    > :   The importable name of a module in the graph, e.g. `'mypackage'` or
    ...
    >
    > raises:
    > :   `ValueError` if the module is a squashed module, as by definition it represents both itself and all
    >     of its descendants.

ImportGraph.find\_descendants(*module*)[](#ImportGraph.find_descendants "Link to this definition")
:   Return all the descendants of the module, i.e. the modules that have a dotted module name that is below
    ...
    > param str module:
    > :   The importable name of the module, e.g. `'mypackage'` or `'mypackage.foo.one'`. As with
...
ImportGraph.find\_matching\_modules(*expression*)[](#ImportGraph.find_matching_modules "Link to this definition")
:   Find all modules matching the passed expression (see [Module expressions](#module-expressions)).
...
## Methods for analysing direct imports[](#methods-for-analysing-direct-imports "Link to this heading")

ImportGraph.direct\_import\_exists(*importer*, *imported*, *as\_packages=False*)[](#ImportGraph.direct_import_exists "Link to this definition")
:   Parameters:
    :   * **importer** (*str*) – A module name.
        * **imported** (*str*) – A module name.
        * **as\_packages** (*bool*) – Whether or not to treat the supplied modules as individual modules, or as entire
...
ImportGraph.find\_modules\_directly\_imported\_by(*module*)[](#ImportGraph.find_modules_directly_imported_by "Link to this definition")
:   Parameters:
...
ImportGraph.find\_modules\_that\_directly\_import(*module*)[](#ImportGraph.find_modules_that_directly_import "Link to this definition")
:   Parameters:
    :   **module** (*str*) – A module name.
...
ImportGraph.get\_import\_details(*importer*, *imported*)[](#ImportGraph.get_import_details "Link to this definition")
...
ImportGraph.count\_imports()[](#ImportGraph.count_imports "Link to this definition")
:   Returns:
    :   The number of imports in the graph. For backward compatibility reasons, `count_imports` does not actually
...
ImportGraph.find\_matching\_direct\_imports(*import\_expression*)[](#ImportGraph.find_matching_direct_imports "Link to this definition")
:   Find all direct imports matching the passed import expression.
    ...
    Parameters:
    :   **import\_expression** (*str*) – An expression in the form `"importer_expression -> imported_expression"`,
        where each expression is a module expression (see [Module expressions](#module-expressions)).
...
## Methods for analysing import chains[](#methods-for-analysing-import-chains "Link to this heading")

ImportGraph.find\_downstream\_modules(*module*, *as\_package=False*)[](#ImportGraph.find_downstream_modules "Link to this definition")
:   Parameters:
    :   * **module** (*str*) – A module name.
        * **as\_package** (*bool*) – Whether or not to treat the supplied module as an individual module,
...
ImportGraph.find\_upstream\_modules(*module*, *as\_package=False*)[](#ImportGraph.find_upstream_modules "Link to this definition")
:   Parameters:
    :   * **module** (*str*) – A module name.
        * **as\_package** (*bool*) – Whether or not to treat the supplied module as an individual module,
...
ImportGraph.find\_shortest\_chain(*importer*, *imported*, *as\_packages=False*)[](#ImportGraph.find_shortest_chain "Link to this definition")
:   Parameters:
    :   * **importer** (*str*) – The module at the start of a potential chain of imports between `importer` and `imported`
        ...
        * **as\_packages** (*bool*) – Whether to treat the supplied modules as individual modules,
...
ImportGraph.find\_shortest\_chains(*importer*, *imported*, *as\_packages=True*)[](#ImportGraph.find_shortest_chains "Link to this definition")
:   Parameters:
    :   * **importer** (*str*) – A module or subpackage within the graph.
        * **imported** (*str*) – Another module or subpackage within the graph.
        * **as\_packages** (*bool*) – Whether or not to treat the imported and importer as an individual module,
    ...
    Returns:
    :   The shortest import chains that exist between the `importer` and `imported`, and between any modules
...
ImportGraph.chain\_exists(*importer*, *imported*, *as\_packages=False*)[](#ImportGraph.chain_exists "Link to this definition")
:   Parameters:
    ...
    Returns:
    :   Return whether any chain of imports exists between `importer` and `imported`,
        even indirectly; in other words, does `importer` depend on `imported`?

    Return type:
    :   bool

## Higher level analysis[](#higher-level-analysis "Link to this heading")

ImportGraph.find\_illegal\_dependencies\_for\_layers(*layers*, *containers=None*)[](#ImportGraph.find_illegal_dependencies_for_layers "Link to this definition")
:   Find dependencies that don’t conform to the supplied layered architecture.

    Parameters:
    :   * **layers** (*Sequence**[*[*Layer*](#Layer "Layer") *|* *str* *|* *set**[**str**]**]*) – A sequence of layers ordered from the highest to the lowest.
          ...
          passing `independent=False` when instantiating the [`Layer`](#Layer "Layer"). For convenience, if a layer consists
          only of one module name then a string may be passed in place of the [`Layer`](#Layer "Layer") object. Additionally, if
          the layer consists of multiple *independent* modules, that can be passed as a set of strings instead of a
          [`Layer`](#Layer "Layer") object. A closed layer may be created by passing `closed=True` to prevent higher layers
          from importing directly from layers below the closed layer (see [Closed layers](#closed-layers) section below).
          *Any modules specified that don’t exist in the graph will be silently ignored.*
        * **containers** (*set**[**str**]*) – The parent modules of the layers, as absolute names that you could
          import, such as `mypackage.foo`. (Optional.)

    Returns:
    :   The illegal dependencies in the form of a set of [`PackageDependency`](#PackageDependency "PackageDependency") objects. Each package
    ...
    ![Layered architecture.](_images/layers.png)
    ...
    ### Layers containing multiple siblings[](#layers-containing-multiple-siblings "Link to this heading")
    ...
    ![Architecture with a layer containing independent siblings.](_images/layers-independent.png)
    ...
    ![Architecture with a layer containing non-independent siblings.](_images/layers-non-independent.png)
    ...
    ### Closed layers[](#closed-layers "Link to this heading")
    ...
    ### Return value[](#return-value "Link to this heading")

    The method returns a set of [`PackageDependency`](#PackageDependency "PackageDependency") objects that describe different illegal imports.

    Note: each returned [`PackageDependency`](#PackageDependency "PackageDependency") does not include all possible illegal [`Route`](#Route "Route") objects.
    Instead, once an illegal [`Route`](#Route "Route") is found, the algorithm will temporarily remove it from the graph before continuing
...
*class* Layer[](#Layer "Link to this definition")
:   A layer within a layered architecture.

    module\_tails[](#Layer.module_tails "Link to this definition")
    ...
    independent[](#Layer.independent "Link to this definition")

    `bool`: Whether the sibling modules within this layer are required to be independent.

*class* PackageDependency[](#PackageDependency "Link to this definition")
:   A collection of import dependencies from one Python package to another.

    importer[](#PackageDependency.importer "Link to this definition")

    `str`: The full name of the package within which all the routes start; the downstream package.
    E.g. “mypackage.foo”.

    imported[](#PackageDependency.imported "Link to this definition")

    `str`: The full name of the package within which all the routes end; the upstream package.
    E.g. “mypackage.bar”.

    routes[](#PackageDependency.routes "Link to this definition")

    `frozenset[grimp.Route]`: A set of [`Route`](#Route "Route") objects from importer to imported.

*class* Route[](#Route "Link to this definition")
:   A set of import chains that share the same middle.
    ...
    heads[](#Route.heads "Link to this definition")
    :   `frozenset[str]`: The importer modules at the start of the chain.

    middle[](#Route.middle "Link to this definition")
    :   `tuple[str]`: A sequence of imports that link the head modules to the tail modules.

    tails[](#Route.tails "Link to this definition")
    :   `frozenset[str]`: Imported modules at the end of the chain.

ImportGraph.nominate\_cycle\_breakers(*package*)[](#ImportGraph.nominate_cycle_breakers "Link to this definition")
:   Choose an approximately minimal set of dependencies that, if removed, would make the package locally acyclic.
    ...
    * ‘Dependency cycles’ mean cycles between the *squashed* children (see [Terminology](#terminology) above).
    ...
    3. Calculate the approximate
       [minimum weighted feedback arc set](https://en.wikipedia.org/wiki/Feedback_arc_set).
    ...
    Parameters:
    :   **package** (*str*) – The package in the graph to check for cycles. If a module with no children is passed,
        an empty set is returned.

    Returns:
    :   A set of imports that, if removed, would make the imports between the the children of the supplied
        package acyclic.

    Return type:
    :   `set[tuple[str, str]]`. In each import tuple, the first element is the importer module and the second
        is the imported.

## Methods for manipulating the graph[](#methods-for-manipulating-the-graph "Link to this heading")

ImportGraph.add\_module(*module*, *is\_squashed=False*)[](#ImportGraph.add_module "Link to this definition")
:   Add a module to the graph.

    Parameters:
    :   * **module** (*str*) – The name of a module, for example `'mypackage.foo'`.
        * **is\_squashed** (*bool*) – If True, the module should be treated as a ‘squashed module’ (see [Terminology](#terminology) above).

    Returns:
    :   None

ImportGraph.remove\_module(*module*)[](#ImportGraph.remove_module "Link to this definition")
...
ImportGraph.add\_import(*importer*, *imported*, *line\_number=None*, *line\_contents=None*)[](#ImportGraph.add_import "Link to this definition")
...
ImportGraph.remove\_import(*importer*, *imported*)[](#ImportGraph.remove_import "Link to this definition")
...
ImportGraph.squash\_module(*module*)[](#ImportGraph.squash_module "Link to this definition")
:   ‘Squash’ a module in the graph (see [Terminology](#terminology) above).
...
ImportGraph.is\_module\_squashed(*module*)[](#ImportGraph.is_module_squashed "Link to this definition")
:   Return whether a module present in the graph is ‘squashed’ (see [Terminology](#terminology) above).
...
## Module expressions[](#module-expressions "Link to this heading")
