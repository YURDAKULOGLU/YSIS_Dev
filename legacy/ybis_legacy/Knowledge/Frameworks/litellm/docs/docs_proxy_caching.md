...
#### Step 1: Add `cache` to the config.yaml[​](#step-1-add-cache-to-the-configyaml-1 "Direct link to step-1-add-cache-to-the-configyaml-1")
...
#### Step 2: Add Qdrant Credentials to your .env[​](#step-2-add-qdrant-credentials-to-your-env "Direct link to Step 2: Add Qdrant Credentials to your .env")
...
#### Step 3: Run proxy with config[​](#step-3-run-proxy-with-config-1 "Direct link to Step 3: Run proxy with config")

```
$ litellm --config /path/to/config.yaml
```

#### Step 4. Test it[​](#step-4-test-it "Direct link to Step 4. Test it")
...
#### Step 1: Add `cache` to the config.yaml[​](#step-1-add-cache-to-the-configyaml-2 "Direct link to step-1-add-cache-to-the-configyaml-2")
...
#### Step 2: Run proxy with config[​](#step-2-run-proxy-with-config "Direct link to Step 2: Run proxy with config")
...
#### Step 1: Add `cache` to the config.yaml[​](#step-1-add-cache-to-the-configyaml-3 "Direct link to step-1-add-cache-to-the-configyaml-3")
...
#### Step 2: Add Redis Credentials to .env[​](#step-2-add-redis-credentials-to-env-1 "Direct link to Step 2: Add Redis Credentials to .env")
...
#### Step 3: Run proxy with config[​](#step-3-run-proxy-with-config-2 "Direct link to Step 3: Run proxy with config")

```
$ litellm --config /path/to/config.yaml
```

#### Step 1: Add `cache` to the config.yaml[​](#step-1-add-cache-to-the-configyaml-4 "Direct link to step-1-add-cache-to-the-configyaml-4")
...
#### Step 2: Run proxy with config[​](#step-2-run-proxy-with-config-1 "Direct link to Step 2: Run proxy with config")

```
$ litellm --config /path/to/config.yaml
```

#### Step 1: Add `cache` to the config.yaml[​](#step-1-add-cache-to-the-configyaml-5 "Direct link to step-1-add-cache-to-the-configyaml-5")
...
#### Step 2: Run proxy with config[​](#step-2-run-proxy-with-config-2 "Direct link to Step 2: Run proxy with config")
...
* [Usage](#usage)
  + [Basic](#basic)
  + [Dynamic Cache Controls](#dynamic-cache-controls)
  + [`ttl`](#ttl-1)
  + [`s-maxage`](#s-maxage)
  + [`no-cache`](#no-cache)
  + [`no-store`](#no-store)
  + [`namespace`](#namespace-1)
* [Set cache for proxy, but not on the actual llm api call](#set-cache-for-proxy-but-not-on-the-actual-llm-api-call)
* [Debugging Caching - `/cache/ping`](#debugging-caching---cacheping)
* [Advanced](#advanced)
  + [Control Call Types Caching is on for - (`/chat/completion`, `/embeddings`, etc.)](#control-call-types-caching-is-on-for---chatcompletion-embeddings-etc)
  + [Set Cache Params on config.yaml](#set-cache-params-on-configyaml)
  + [Deleting Cache Keys - `/cache/delete`](#deleting-cache-keys---cachedelete)
  + [\*\*Set Caching Default Off - Opt in only \*\*](#set-caching-default-off---opt-in-only-)
* [Redis max\_connections](#redis-max_connections)
* [Supported `cache_params` on proxy config.yaml](#supported-cache_params-on-proxy-configyaml)
* [Provider-Specific Optional Parameters Caching](#provider-specific-optional-parameters-caching)
  + [Enable Provider-Specific Parameter Caching](#enable-provider-specific-parameter-caching)
* [Advanced - user api key cache ttl](#advanced---user-api-key-cache-ttl)