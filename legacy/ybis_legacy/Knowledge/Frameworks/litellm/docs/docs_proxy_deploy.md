...
Facing issues with pulling the docker image? Email us at [support@berri.ai](mailto:support@berri.ai).
...
### Docker Run[​](#docker-run "Direct link to Docker Run")
...
### Docker Run - CLI Args[​](#docker-run---cli-args "Direct link to Docker Run - CLI Args")
...
### Use litellm as a base image[​](#use-litellm-as-a-base-image "Direct link to Use litellm as a base image")
...
### Build from litellm `pip` package[​](#build-from-litellm-pip-package "Direct link to build-from-litellm-pip-package")
...
#### Step 1. Pull the litellm helm chart[​](#step-1-pull-the-litellm-helm-chart "Direct link to Step 1. Pull the litellm helm chart")
...
#### Step 2. Unzip litellm helm[​](#step-2-unzip-litellm-helm "Direct link to Step 2. Unzip litellm helm")
...
#### Step 3. Install litellm helm[​](#step-3-install-litellm-helm "Direct link to Step 3. Install litellm helm")

```
helm install lite-helm ./litellm-helm
```

#### Step 4. Expose the service to localhost[​](#step-4-expose-the-service-to-localhost "Direct link to Step 4. Expose the service to localhost")
...
#### Make LLM API Requests[​](#make-llm-api-requests "Direct link to Make LLM API Requests")
...
### Deploy with Database[​](#deploy-with-database "Direct link to Deploy with Database")

##### Docker, Kubernetes, Helm Chart[​](#docker-kubernetes-helm-chart "Direct link to Docker, Kubernetes, Helm Chart")
...
#### Step 1. Create deployment.yaml[​](#step-1-create-deploymentyaml "Direct link to Step 1. Create deployment.yaml")
...
#### Step 2. Create service.yaml[​](#step-2-create-serviceyaml "Direct link to Step 2. Create service.yaml")
...
#### Step 3. Start server[​](#step-3-start-server "Direct link to Step 3. Start server")
...
#### Step 1. Clone the repository[​](#step-1-clone-the-repository "Direct link to Step 1. Clone the repository")

```
git clone https://github.com/BerriAI/litellm.git
```

#### Step 2. Deploy with Helm[​](#step-2-deploy-with-helm "Direct link to Step 2. Deploy with Helm")
...
#### Step 3. Expose the service to localhost[​](#step-3-expose-the-service-to-localhost "Direct link to Step 3. Expose the service to localhost")
...
#### Step 1. Pull the litellm helm chart[​](#step-1-pull-the-litellm-helm-chart-1 "Direct link to Step 1. Pull the litellm helm chart")
...
#### Step 2. Unzip litellm helm[​](#step-2-unzip-litellm-helm-1 "Direct link to Step 2. Unzip litellm helm")
...
#### Step 3. Install litellm helm[​](#step-3-install-litellm-helm-1 "Direct link to Step 3. Install litellm helm")

```
helm install lite-helm ./litellm-helm
```

#### Step 4. Expose the service to localhost[​](#step-4-expose-the-service-to-localhost-1 "Direct link to Step 4. Expose the service to localhost")
...
### Deploy with Redis[​](#deploy-with-redis "Direct link to Deploy with Redis")
...
### Deploy with Database + Redis[​](#deploy-with-database--redis "Direct link to Deploy with Database + Redis")
...
### (Non Root) - without Internet Connection[​](#non-root---without-internet-connection "Direct link to (Non Root) - without Internet Connection")
...
## Advanced Deployment Settings[​](#advanced-deployment-settings "Direct link to Advanced Deployment Settings")

### 1. Custom server root path (Proxy base url)[​](#1-custom-server-root-path-proxy-base-url "Direct link to 1. Custom server root path (Proxy base url)")

Refer to [Custom Root Path](/docs/proxy/custom_root_ui) for more details.

### 2. SSL Certification[​](#2-ssl-certification "Direct link to 2. SSL Certification")
...
### 3. Http/2 with Hypercorn[​](#3-http2-with-hypercorn "Direct link to 3. Http/2 with Hypercorn")
...
### 4. Keepalive Timeout[​](#4-keepalive-timeout "Direct link to 4. Keepalive Timeout")
...
### Restart Workers After N Requests[​](#restart-workers-after-n-requests "Direct link to Restart Workers After N Requests")
...
### 5. config.yaml file on s3, GCS Bucket Object/url[​](#5-configyaml-file-on-s3-gcs-bucket-objecturl "Direct link to 5. config.yaml file on s3, GCS Bucket Object/url")
...
### 6. Disable pulling live model prices[​](#6-disable-pulling-live-model-prices "Direct link to 6. Disable pulling live model prices")
...
## Platform-specific Guide[​](#platform-specific-guide "Direct link to Platform-specific Guide")
...
### Terraform-based ECS Deployment[​](#terraform-based-ecs-deployment "Direct link to Terraform-based ECS Deployment")
...
### Kubernetes (AWS EKS)[​](#kubernetes-aws-eks "Direct link to Kubernetes (AWS EKS)")
...
### AWS Cloud Formation Stack[​](#aws-cloud-formation-stack "Direct link to AWS Cloud Formation Stack")
...
#### Using AWS Cloud Formation Stack[​](#using-aws-cloud-formation-stack "Direct link to Using AWS Cloud Formation Stack")
...
#### 1. Create the CloudFormation Stack:[​](#1-create-the-cloudformation-stack "Direct link to 1. Create the CloudFormation Stack:")
...
#### 2. Get the Database URL:[​](#2-get-the-database-url "Direct link to 2. Get the Database URL:")

Once the stack is created, get the DatabaseURL of the Database resource, copy this value

#### 3. Connect to the EC2 Instance and deploy litellm on the EC2 container[​](#3-connect-to-the-ec2-instance-and-deploy-litellm-on-the-ec2-container "Direct link to 3. Connect to the EC2 Instance and deploy litellm on the EC2 container")
...
#### 4. Access the Application:[​](#4-access-the-application "Direct link to 4. Access the Application:")
...
1. Fork this repo - [github.com/BerriAI/example\_litellm\_gcp\_cloud\_run](https://github.com/BerriAI/example_litellm_gcp_cloud_run)
...
#### Testing your deployed proxy[​](#testing-your-deployed-proxy "Direct link to Testing your deployed proxy")
...
## Extras[​](#extras "Direct link to Extras")
...
### IAM-based Auth for RDS DB[​](#iam-based-auth-for-rds-db "Direct link to IAM-based Auth for RDS DB")
...
### ✨ Blocking web crawlers[​](#-blocking-web-crawlers "Direct link to ✨ Blocking web crawlers")
...
## Deployment FAQ[​](#deployment-faq "Direct link to Deployment FAQ")
...
* [Quick Start](#quick-start)
  ...
  + [Docker Run - CLI Args](#docker-run---cli-args)
  + [Use litellm as a base image](#use-litellm-as-a-base-image)
...
* [Deployment Options](#deployment-options)
  ...
  + [Deploy with Redis](#deploy-with-redis)
  + [Deploy with Database + Redis](#deploy-with-database--redis)
  + [(Non Root) - without Internet Connection](#non-root---without-internet-connection)
* [Advanced Deployment Settings](#advanced-deployment-settings)
  + [1. Custom server root path (Proxy base url)](#1-custom-server-root-path-proxy-base-url)
  + [2. SSL Certification](#2-ssl-certification)
  + [3. Http/2 with Hypercorn](#3-http2-with-hypercorn)
  + [4. Keepalive Timeout](#4-keepalive-timeout)
  + [Restart Workers After N Requests](#restart-workers-after-n-requests)
  + [5. config.yaml file on s3, GCS Bucket Object/url](#5-configyaml-file-on-s3-gcs-bucket-objecturl)
  + [6. Disable pulling live model prices](#6-disable-pulling-live-model-prices)
* [Platform-specific Guide](#platform-specific-guide)
  + [Terraform-based ECS Deployment](#terraform-based-ecs-deployment)
  + [Kubernetes (AWS EKS)](#kubernetes-aws-eks)
  + [AWS Cloud Formation Stack](#aws-cloud-formation-stack)
  + [Google Cloud Run](#google-cloud-run)
  + [Render](#render)
  + [Railway](#railway)
* [Extras](#extras)
  + [Docker compose](#docker-compose)
  + [IAM-based Auth for RDS DB](#iam-based-auth-for-rds-db)
  + [✨ Blocking web crawlers](#-blocking-web-crawlers)
* [Deployment FAQ](#deployment-faq)