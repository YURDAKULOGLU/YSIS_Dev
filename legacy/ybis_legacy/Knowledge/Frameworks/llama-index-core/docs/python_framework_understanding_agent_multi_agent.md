 Multi-agent patterns in LlamaIndex | LlamaIndex Python Documentation
...
[Section titled “Pattern 1 – AgentWorkflow (i.e. linear “swarm” pattern)”](#pattern-1--agentworkflow-ie-linear-swarm-pattern)
...
`AgentWorkflow` is itself a [Workflow](/python/llamaagents/workflows) pre-configured to understand agents, state and tool-calling. You supply an *array* of one or more agents, tell it which one should start, and it will:
...
[Section titled “Pattern 2 – Orchestrator agent (sub-agents as tools)”](#pattern-2--orchestrator-agent-sub-agents-as-tools)
...
You can see the full example in the [agents\_as\_tools notebook](/python/examples/agent/agents_as_tools).
...
[Section titled “Pattern 3 – Custom planner (DIY prompting + parsing)”](#pattern-3--custom-planner-diy-prompting--parsing)
...
Below is a *minimal* sketch of a workflow that can plan, execute a plan, and see if any further steps are needed. You can see the full example in the [custom\_multi\_agent notebook](/python/examples/agent/custom_multi_agent).
