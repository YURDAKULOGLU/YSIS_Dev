 Run Your Workflow as a Server | LlamaIndex Python Documentation
...
[Section titled “Workflow Debugger UI”](#workflow-debugger-ui)
...
![Workflow Debugger UI](/python/_astro/ui_sample.CnjiOwlC_1LUmcI.webp)
...
[Section titled “Handling “Hidden” Events”](#handling-hidden-events)
...
[Section titled “Running a Workflow (/run)”](#running-a-workflow-run)
...
[Section titled “Running a Workflow Asynchronously (/run-nowait)”](#running-a-workflow-asynchronously-run-nowait)
...
[Section titled “Streaming events (GET /events/{handler\_id})”](#streaming-events-get-eventshandler_id)

> *This endpoint only works if you previously started a workflow asynchronously with `/run-nowait`*
...
> *We are working to improve both these aspects, so changes in the server behavior might be expected*

## Getting the result from a workflow execution (`/results/{handler_id}`)

[Section titled “Getting the result from a workflow execution (/results/{handler\_id})”](#getting-the-result-from-a-workflow-execution-resultshandler_id)
...
[Section titled “Sending an event (POST /events/{handler\_id})”](#sending-an-event-post-eventshandler_id)
...
[Section titled “Canceling a workflow run (/handlers/{handler\_id}/cancel)”](#canceling-a-workflow-run-handlershandler_idcancel)
...
[Section titled “Using WorkflowClient to Interact with Servers”](#using-workflowclient-to-interact-with-servers)
