...
* [Help & Guides](../resources.html)
* Code Snippets and Recipes for Loguru

---

# Code Snippets and Recipes for Loguru[](#code-snippets-and-recipes-for-loguru "Link to this heading")

## Security considerations when using Loguru[](#security-considerations-when-using-loguru "Link to this heading")

Firstly, if you use [`pickle`](https://docs.python.org/3/library/pickle.html#module-pickle "(in Python v3.13)") to load log messages (e.g. from the network), make sure the source is trustable or sign the data to verify its authenticity before deserializing it. If you do not take these precautions, malicious code could be executed by an attacker. You can read more details in this article: [What’s so dangerous about pickles?](https://intoli.com/blog/dangerous-pickles/)
...
Another thing you should consider is to change the access permissions of your log file. Loguru creates files using the built-in [`open()`](https://docs.python.org/3/library/functions.html#open "(in Python v3.13)") function, which means by default they might be read by a different user than the owner. If this is not desirable, be sure to modify the default access rights.
...
## Avoiding logs to be printed twice on the terminal[](#avoiding-logs-to-be-printed-twice-on-the-terminal "Link to this heading")

The logger is pre-configured for convenience with a default handler which writes messages to [`sys.stderr`](https://docs.python.org/3/library/sys.html#sys.stderr "(in Python v3.13)"). You should [`remove()`](../api/logger.html#loguru._logger.Logger.remove "loguru._logger.Logger.remove") it first if you plan to [`add()`](../api/logger.html#loguru._logger.Logger.add "loguru._logger.Logger.add") another handler logging messages to the console, otherwise you may end up with duplicated logs.
...
## Changing the level of an existing handler[](#changing-the-level-of-an-existing-handler "Link to this heading")
...
Alternatively, you can combine the [`bind()`](../api/logger.html#loguru._logger.Logger.bind "loguru._logger.Logger.bind") method with the `filter` argument to provide a function dynamically filtering logs based on their level:
...
## Configuring Loguru to be used by a library or an application[](#configuring-loguru-to-be-used-by-a-library-or-an-application "Link to this heading")
...
## Sending and receiving log messages across network or processes[](#sending-and-receiving-log-messages-across-network-or-processes "Link to this heading")
...
This can be achieved using a custom sink for the client and [`patch()`](../api/logger.html#loguru._logger.Logger.patch "loguru._logger.Logger.patch") for the server.
...
Another possibility is to use a third party library like [`zmq`](https://github.com/zeromq/pyzmq) for example.
...
* Use [`sys.stderr`](https://docs.python.org/3/library/sys.html#sys.stderr "(in Python v3.13)") instead of [`sys.stdout`](https://docs.python.org/3/library/sys.html#sys.stdout "(in Python v3.13)") (the former will escape faulty characters rather than raising exception)
* Set the [`PYTHONIOENCODING`](https://docs.python.org/3/using/cmdline.html#envvar-PYTHONIOENCODING "(in Python v3.13)") environment variable to `utf-8`
* Call [`sys.stdout.reconfigure()`](https://docs.python.org/3/library/io.html#io.TextIOWrapper.reconfigure "(in Python v3.13)") with `encoding='utf-8'` and / or `errors='backslashreplace'`

If you are using a file sink, you can configure the `errors` or `encoding` parameter while adding the handler like `logger.add("file.log", encoding="utf8")` for example. All additional `**kwargs` argument are passed to the built-in [`open()`](https://docs.python.org/3/library/functions.html#open "(in Python v3.13)") function.

For other types of handlers, you have to check if there is a way to parametrize encoding or fallback policy.

## Logging entry and exit of functions with a decorator[](#logging-entry-and-exit-of-functions-with-a-decorator "Link to this heading")
...
## Using logging function based on custom added levels[](#using-logging-function-based-on-custom-added-levels "Link to this heading")

After adding a new level, it’s habitually used with the [`log()`](../api/logger.html#loguru._logger.Logger.log "loguru._logger.Logger.log") function:
...
## Setting permissions on created log files[](#setting-permissions-on-created-log-files "Link to this heading")
...
Note that the provided mode will be masked out by the OS [umask](https://en.wikipedia.org/wiki/Umask) value (describing which bits are *not* to be set when creating a file or directory). This value is conventionally equals to `0o022`, which means specifying a `0o666` mode will result in a `0o666 - 0o022 = 0o644` file permission in this case (which is actually the default). It is possible to change the umask value by first calling [`os.umask()`](https://docs.python.org/3/library/os.html#os.umask "(in Python v3.13)"), but this needs to be done with careful consideration, as it changes the value globally and can cause security issues.

## Preserving an `opt()` parameter for the whole module[](#preserving-an-opt-parameter-for-the-whole-module "Link to this heading")
...
## Serializing log messages using a custom function[](#serializing-log-messages-using-a-custom-function "Link to this heading")
...
You can also use [`patch()`](../api/logger.html#loguru._logger.Logger.patch "loguru._logger.Logger.patch") for this, so the serialization function will be called only once in case you want to use it in multiple sinks:
...
## Rotating log file based on both size and time[](#rotating-log-file-based-on-both-size-and-time "Link to this heading")
...
## Adapting colors and format of logged messages dynamically[](#adapting-colors-and-format-of-logged-messages-dynamically "Link to this heading")

It is possible to customize the colors of your logs thanks to several [markup tags](../api/logger.html#color). Those are used to configure the `format` of your handler. By creating a appropriate formatting function, you can easily define colors depending on the logged message.
...
## Dynamically formatting messages to properly align values with padding[](#dynamically-formatting-messages-to-properly-align-values-with-padding "Link to this heading")
...
## Customizing the formatting of exceptions[](#customizing-the-formatting-of-exceptions "Link to this heading")
...
## Manipulating newline terminator to write multiple logs on the same line[](#manipulating-newline-terminator-to-write-multiple-logs-on-the-same-line "Link to this heading")

You can temporarily log a message on a continuous line by combining the use of [`bind()`](../api/logger.html#loguru._logger.Logger.bind "loguru._logger.Logger.bind"), [`opt()`](../api/logger.html#loguru._logger.Logger.opt "loguru._logger.Logger.opt") and a custom `format` function. This is especially useful if you want to illustrate a step-by-step process in progress, for example:
...
## Capturing standard `stdout`, `stderr` and `warnings`[](#capturing-standard-stdout-stderr-and-warnings "Link to this heading")

The use of logging should be privileged over [`print()`](https://docs.python.org/3/library/functions.html#print "(in Python v3.13)"), yet, it may happen that you don’t have plain control over code executed in your application. If you wish to capture standard output, you can suppress [`sys.stdout`](https://docs.python.org/3/library/sys.html#sys.stdout "(in Python v3.13)") (and [`sys.stderr`](https://docs.python.org/3/library/sys.html#sys.stderr "(in Python v3.13)")) with a custom stream object using [`contextlib.redirect_stdout()`](https://docs.python.org/3/library/contextlib.html#contextlib.redirect_stdout "(in Python v3.13)"). You have to take care of first removing the default handler, and not adding a new stdout sink once redirected or that would cause dead lock (you may use [`sys.__stdout__`](https://docs.python.org/3/library/sys.html#sys.__stdout__ "(in Python v3.13)") instead):
...
You may also capture warnings emitted by your application by replacing [`warnings.showwarning()`](https://docs.python.org/3/library/warnings.html#warnings.showwarning "(in Python v3.13)"):
...
Alternatively, if you want to emit warnings based on logged messages, you can simply use [`warnings.warn()`](https://docs.python.org/3/library/warnings.html#warnings.warn "(in Python v3.13)") as a sink:

```
logger.add(warnings.warn, format="{message}", filter=lambda record: record["level"].name == "WARNING")
```

## Circumventing modules whose `__name__` value is absent[](#circumventing-modules-whose-name-value-is-absent "Link to this heading")

Loguru makes use of the global variable `__name__` to determine from where the logged message is coming from. However, it may happen in very specific situation (like some Dask distributed environment) that this value is not set. In such case, Loguru will use `None` to make up for the lack of the value. This implies that if you want to [`disable()`](../api/logger.html#loguru._logger.Logger.disable "loguru._logger.Logger.disable") messages coming from such special module, you have to explicitly call `logger.disable(None)`.

Similar considerations should be taken into account while dealing with the `filter` attribute. As `__name__` is missing, Loguru will assign the `None` value to the `record["name"]` entry. It also means that once formatted in your log messages, the `{name}` token will be equals to `"None"`. This can be worked around by manually overriding the `record["name"]` value using [`patch()`](../api/logger.html#loguru._logger.Logger.patch "loguru._logger.Logger.patch") from inside the faulty module:
...
## Interoperability with `tqdm` iterations[](#interoperability-with-tqdm-iterations "Link to this heading")
...
## Using Loguru’s `logger` within a Cython module[](#using-loguru-s-logger-within-a-cython-module "Link to this heading")
...
You can update the default `format` of your handlers and omit the uninteresting fields. You can also tries to [`patch()`](../api/logger.html#loguru._logger.Logger.patch "loguru._logger.Logger.patch") the `logger` to manually add information you may know about the caller, for example:
...
## Creating independent loggers with separate set of handlers[](#creating-independent-loggers-with-separate-set-of-handlers "Link to this heading")
...
For example, supposing you want to split your logs in two files based on an arbitrary identifier, you can achieve that by combining [`bind()`](../api/logger.html#loguru._logger.Logger.bind "loguru._logger.Logger.bind") and `filter`:
...
## Compatibility with `multiprocessing` using `enqueue` argument[](#compatibility-with-multiprocessing-using-enqueue-argument "Link to this heading")

On Linux, thanks to [`os.fork()`](https://docs.python.org/3/library/os.html#os.fork "(in Python v3.13)") there is no pitfall while using the `logger` inside another process started by the [`multiprocessing`](https://docs.python.org/3/library/multiprocessing.html#module-multiprocessing "(in Python v3.13)") module. The child process will automatically inherit added handlers, the `enqueue=True` parameter is optional but is recommended as it would avoid concurrent access of your sink:
...
The [`multiprocessing`](https://docs.python.org/3/library/multiprocessing.html#module-multiprocessing "(in Python v3.13)") library is also commonly used to start a pool of workers using for example [`map()`](https://docs.python.org/3/library/multiprocessing.html#multiprocessing.pool.Pool.map "(in Python v3.13)") or [`apply()`](https://docs.python.org/3/library/multiprocessing.html#multiprocessing.pool.Pool.apply "(in Python v3.13)"). Again, it will work flawlessly on Linux, but it will require some tinkering on Windows. You will probably not be able to pass the `logger` as an argument for your worker functions because it needs to be picklable, but although handlers added using `enqueue=True` are “inheritable”, they are not “picklable”. Instead, you will need to make use of the `initializer` and `initargs` parameters while creating the [`Pool`](https://docs.python.org/3/library/multiprocessing.html#multiprocessing.pool.Pool "(in Python v3.13)") object in a way allowing your workers to access the shared `logger`. You can either assign it to a class attribute or override the global logger of your child processes:
...
## Testing logging[](#testing-logging "Link to this heading")

Logging calls can be tested using [`logot`](https://logot.readthedocs.io/), a high-level log testing library with built-in support for Loguru:
...
See [using logot with Loguru](https://logot.readthedocs.io/latest/integrations/loguru.html) for more information
about [configuring pytest](https://logot.readthedocs.io/latest/integrations/loguru.html#enabling-for-pytest)
and [configuring unittest](https://logot.readthedocs.io/latest/integrations/loguru.html#enabling-for-unittest).

Note

When migrating an existing project from standard [`logging`](https://docs.python.org/3/library/logging.html#module-logging "(in Python v3.13)"), it can be useful to migrate your existing test
cases too. See [migrating assertLogs()](migration.html#migration-assert-logs) and [migrating caplog](migration.html#migration-caplog)
