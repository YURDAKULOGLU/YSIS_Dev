...
CustomFilter
++++++++++++
CustomFilters can have one or two conditions which will operate either independently (the default), or combined by setting the ``and\_`` attribute. Filter can use the following operators: ``equal, lessThan, lessThanOrEqual, notEqual, greaterThanOrEqual, greaterThan``.
