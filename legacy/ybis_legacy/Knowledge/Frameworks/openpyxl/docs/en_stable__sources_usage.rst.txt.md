...
>>> chart.y\_axis.title = 'Height (cm)'
>>> chart.x\_axis.title = 'Tree Type'
>>> chart.legend = None
...
.. :: doctest exercise-1
>>> data = Reference(ws, min\_col=3, min\_row=2, max\_row=4, max\_col=3)
>>> categories = Reference(ws, min\_col=1, min\_row=2, max\_row=4, max\_col=1)
>>> chart.add\_data(data)
>>> chart.set\_categories(categories)
Finally we can add it to the sheet.
.. :: doctest exercise-1
>>> ws.add\_chart(chart, "E1")
