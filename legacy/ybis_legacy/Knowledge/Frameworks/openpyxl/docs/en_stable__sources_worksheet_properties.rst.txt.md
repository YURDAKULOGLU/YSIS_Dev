...
You can use :class:`ws.sheet\_view` to set sheet attributes such as zoom, show formulas or if the tab is selected. `ws.sheet\_view` returns the first view.
.. :: doctest
>>> from openpyxl.workbook import Workbook
>>>
>>> wb = Workbook()
>>> ws = wb.active
>>>
>>> ws.sheet\_view.zoomScale = 85 # Sets 85% zoom
>>> ws.sheet\_view.showFormulas = True
>>> ws.sheet\_view.tabSelected = True
...
>>> ws = wb.create\_sheet()
>>> ws.column\_dimensions.group('A','D', hidden=True)
>>> ws.row\_dimensions.group(1,10, hidden=True)
>>> wb.save('group.xlsx')