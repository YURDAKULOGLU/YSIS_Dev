Earnings Reports to CSV - Outlines
...
[Skip to content](#extracting-financial-data-from-earnings-reports)
...
Pretty hairy, right? Thankfully, we have a simple function to construct this regex for you. The regex defines a header line, followed by a data line that repeats for each row of data we want to extract. Passing the regex to `outlines.Generator` will produce a function that will **always** produce a CSV string that is consistent with the regex.
