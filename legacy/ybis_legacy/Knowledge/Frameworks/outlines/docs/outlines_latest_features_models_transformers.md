...
If you provide a processor instead of a tokenizer for the second argument of the `from_transformers` function, you would get a `TransformersMultiModal` instance. See the [TransformersMultiModal model documentation](../transformers_multimodal/) for more information on using multimodal models in Outlines.
