How to write interactive workflows - Prefect
...
* [Pause or suspend a flow until it receives input](#pause-or-suspend-a-flow-until-it-receives-input)
* [Wait for input](#wait-for-input)
* [Provide initial data](#provide-initial-data)
* [Provide a description with runtime data](#provide-a-description-with-runtime-data)
* [Handle custom validation](#handle-custom-validation)
* [Send and receive input at runtime](#send-and-receive-input-at-runtime)
* [Receiving input](#receiving-input)
* [Keep track of inputs you’ve already seen](#keep-track-of-inputs-you%E2%80%99ve-already-seen)
* [Respond to the input’s sender](#respond-to-the-input%E2%80%99s-sender)
...
**Types can you pass for `wait_for_input`**When you pass a built-in type such as `int` as an argument for the `wait_for_input` parameter to `pause_flow_run` or `suspend_flow_run`,
...
**`BaseModel` classes are upgraded to `RunInput` classes automatically**When you pass a `pydantic.BaseModel` class as the `wait_for_input` argument to `pause_flow_run` or `suspend_flow_run`, Prefect
...
**Calling custom validation runs after the flow resumes**Prefect transforms the type annotations in your `RunInput` or `BaseModel` class to a JSON schema and uses that schema in
...
**Reasons to send or receive input without pausing or suspending**You might want to send or receive input without pausing or suspending in scenarios where the flow run is designed to handle
...
**When to use a `BaseModel` or `RunInput` instead of a built-in type**Most built-in types and collections of built-in types should work with `send_input` and `receive_input`, but there is a caveat with
...
**When to use `with_metadata=True`**The primary uses of accessing the `RunInput` object for a receive input are to respond to the sender with the `RunInput.respond()`
...
**The type used in `receive_input` and `send_input` must match**For a flow to receive input, the sender must use the same type that the receiver is receiving.
...
### [​](#keep-track-of-inputs-you’ve-already-seen) Keep track of inputs you’ve already seen
...
### [​](#respond-to-the-input’s-sender) Respond to the input’s sender
...
**When to send input to a flow run**Send input to a flow run as soon as you have the flow run’s ID. The flow does not have to be receiving input for you to send input.
...
[Workflow cancellation](/v3/advanced/cancel-workflows)[Configure UI forms for validating workflow inputs](/v3/advanced/form-building)
