How to access runtime information - Prefect
...
**Attributes of `prefect.runtime` are `None` if a value is not available**Attributes of `prefect.runtime` will never error when accessed. If a value is not available, the attribute will be `None`.
...
[Add logging](/v3/how-to-guides/workflows/add-logging)[Run work concurrently](/v3/how-to-guides/workflows/run-work-concurrently)
