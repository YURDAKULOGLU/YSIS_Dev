Usage Stats Collection — Ray 2.53.0
...
# Usage Stats Collection[#](#usage-stats-collection "Link to this heading")
...
You will always be able to [disable the usage stats collection](#usage-disable).

For more context, please refer to this [RFC](https://github.com/ray-project/ray/issues/20857).

## What data is collected?[#](#what-data-is-collected "Link to this heading")
...
## How to disable it[#](#how-to-disable-it "Link to this heading")

There are multiple ways to disable usage stats collection before starting a cluster:

1. Add `--disable-usage-stats` option to the command that starts the Ray cluster (e.g., `ray start --head --disable-usage-stats` [command](cli.html#ray-start-doc)).
2. Run [ray disable-usage-stats](../ray-core/api/cli.html#ray-disable-usage-stats-doc) to disable collection for all future clusters. This won’t affect currently running clusters. Under the hood, this command writes `{"usage_stats": true}` to the global config file `~/.ray/config.json`.
3. Set the environment variable `RAY_USAGE_STATS_ENABLED` to 0 (e.g., `RAY_USAGE_STATS_ENABLED=0 ray start --head` [command](cli.html#ray-start-doc)).
4. If you’re using [KubeRay](https://github.com/ray-project/kuberay/), you can add `disable-usage-stats: 'true'` to `.spec.[headGroupSpec|workerGroupSpecs].rayStartParams.`.
...
When a Ray cluster is started via [ray start –head](cli.html#ray-start-doc), [ray up](cli.html#ray-up-doc), [ray submit –start](cli.html#ray-submit-doc) or [ray exec –start](cli.html#ray-exec-doc),
...
2. If the environment variable is not set, it reads the value of key `usage_stats` in the global config file `~/.ray/config.json`: true means enabled and false means disabled.
3. If neither is set and the console is interactive, then the user will be prompted to enable or disable the collection. If the console is non-interactive, usage stats collection will be enabled by default. The decision will be saved to `~/.ray/config.json`, so the prompt is only shown once.
...
## Requesting removal of collected data[#](#requesting-removal-of-collected-data "Link to this heading")
...
**Does the session\_id map to personal data?**
...
The session\_id is logged so that deletion requests can be honored.
...
[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/cluster/usage-stats.rst)