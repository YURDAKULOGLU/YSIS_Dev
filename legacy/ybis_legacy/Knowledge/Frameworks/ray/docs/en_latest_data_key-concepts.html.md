...
## Datasets and blocks[#](#datasets-and-blocks "Link to this heading")
...
1. Create a [`Dataset`](api/dataset.html#ray.data.Dataset "ray.data.Dataset") from external storage or in-memory data.
...
like [`show()`](api/doc/ray.data.Dataset.show.html#ray.data.Dataset.show "ray.data.Dataset.show"). This allows Ray Data to optimize the execution plan
...
Ray Data holds the [`Dataset`](api/dataset.html#ray.data.Dataset "ray.data.Dataset") on the process that triggers execution
(which is usually the entrypoint of the program, referred to as the [driver](../ray-references/glossary.html#term-Driver))
and stores the blocks as objects in Ray’s shared-memory [object store](../ray-core/objects.html#objects-in-ray). Internally, Ray Data can natively handle blocks either
as Pandas `DataFrame` or PyArrow `Table`.

![../_images/dataset-arch-with-blocks.svg](../_images/dataset-arch-with-blocks.svg)

## Operators and Plans[#](#operators-and-plans "Link to this heading")
...
[![../_images/get_execution_plan.svg](../_images/get_execution_plan.svg)](../_images/get_execution_plan.svg)
...
For more details on Ray Tasks and Actors, see [Ray Core Concepts](../ray-core/key-concepts.html#core-key-concepts).

Note

A dataset’s execution plan only runs when you materialize or consume the dataset through operations like [`show()`](api/doc/ray.data.Dataset.show.html#ray.data.Dataset.show "ray.data.Dataset.show").

## Streaming execution model[#](#streaming-execution-model "Link to this heading")
...
[![../_images/streaming-topology.svg](../_images/streaming-topology.svg)](../_images/streaming-topology.svg)
...
Operations like [`ds.sort()`](api/doc/ray.data.Dataset.sort.html#ray.data.Dataset.sort "ray.data.Dataset.sort") and [`ds.groupby()`](api/doc/ray.data.Dataset.groupby.html#ray.data.Dataset.groupby "ray.data.Dataset.groupby") require materializing data, which may impact memory usage for very large datasets.

You can read more about the streaming execution model in this [blog post](https://www.anyscale.com/blog/streaming-distributed-execution-across-cpus-and-gpus).

On this page

[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/data/key-concepts.rst)