Build a tool-using agent — Ray 2.53.0
...
# Build a tool-using agent[#](#build-a-tool-using-agent "Link to this heading")
...
![Overall Architecture](https://anyscale-public-materials.s3.us-west-2.amazonaws.com/agent-template/overall-architecture.svg)

### Benefits of this architecture[#](#benefits-of-this-architecture "Link to this heading")
...
### Key benefits using Ray and Anyscale include:[#](#key-benefits-using-ray-and-anyscale-include "Link to this heading")
...
## Dependencies and compute resource requirements[#](#dependencies-and-compute-resource-requirements "Link to this heading")
...
## Implementation: Building the services[#](#implementation-building-the-services "Link to this heading")

This project consists of several Python scripts that work together to create and serve the agent.

### Step 1: Create the LLM service[#](#step-1-create-the-llm-service "Link to this heading")
...
### Step 2: Create the MCP weather tool service[#](#step-2-create-the-mcp-weather-tool-service "Link to this heading")
...
### Step 3: Create the agent logic[#](#step-3-create-the-agent-logic "Link to this heading")

![Agent Architecture](https://anyscale-public-materials.s3.us-west-2.amazonaws.com/agent-template/agent-architecture.svg)
...
### Step 4: Create the agent deployment script[#](#step-4-create-the-agent-deployment-script "Link to this heading")
...
## Deploy the services[#](#deploy-the-services "Link to this heading")

Now that you’ve reviewed the code, deploy each service to Anyscale.

### Step 5: Deploy the LLM service[#](#step-5-deploy-the-llm-service "Link to this heading")
...
### Step 6: Deploy the weather MCP service[#](#step-6-deploy-the-weather-mcp-service "Link to this heading")
...
### Step 7: Configure the agent[#](#step-7-configure-the-agent "Link to this heading")
...
### Step 8: Deploy the agent service locally[#](#step-8-deploy-the-agent-service-locally "Link to this heading")
...
## Test the agent[#](#test-the-agent "Link to this heading")

### Step 9: Send test requests[#](#step-9-send-test-requests "Link to this heading")

With the agent service running, send requests to the `/chat` endpoint.
The script “helpers/agent\_client\_local.py” sends a request and streams the response:
...
### Terminate the local server[#](#terminate-the-local-server "Link to this heading")
...
### Step 10: Deploy the agent to production on Anyscale[#](#step-10-deploy-the-agent-to-production-on-anyscale "Link to this heading")
...
#### Why deploy to Anyscale[#](#why-deploy-to-anyscale "Link to this heading")
...
#### Deploy the agent service[#](#deploy-the-agent-service "Link to this heading")
...
#### Understanding the deployment output[#](#understanding-the-deployment-output "Link to this heading")
...
#### Test the production agent[#](#test-the-production-agent "Link to this heading")
...
**Note:** The repository also includes the script at “helpers/agent\_client\_anyscale.py” for your reference.
...
### Terminate the production server[#](#terminate-the-production-server "Link to this heading")
...
### Extend your agent[#](#extend-your-agent "Link to this heading")
...
### Optimize for production[#](#optimize-for-production "Link to this heading")
...
### Production best practices[#](#production-best-practices "Link to this heading")
...
[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/ray-overview/examples/langchain_agent_ray_serve/content/README.ipynb)