Tuning XGBoost hyperparameters with Ray Tune — Ray 2.53.0
...
# [Tuning XGBoost hyperparameters with Ray Tune](#id4)[#](#tuning-xgboost-hyperparameters-with-ray-tune "Link to this heading")

[![try-anyscale-quickstart](../../_static/img/run-on-anyscale.svg)](https://console.anyscale.com/register/ha?render_flow=ray&utm_source=ray_docs&utm_medium=docs&utm_campaign=tune-xgboost)
...
[![XGBoost](../../_images/xgboost_logo.png)](https://xgboost.readthedocs.io/en/latest/)
...
## [What is XGBoost](#id5)[#](#what-is-xgboost "Link to this heading")
...
![Single vs. ensemble learning](../../_images/tune-xgboost-ensemble.svg)
...
## [Training a simple XGBoost classifier](#id6)[#](#training-a-simple-xgboost-classifier "Link to this heading")
...
is a [`dict`](https://docs.python.org/3/library/stdtypes.html#dict "(in Python v3.14)") in which you can specify parameters for the XGBoost algorithm. In this
...
## [Scaling XGBoost Training with Ray Train](#id7)[#](#scaling-xgboost-training-with-ray-train "Link to this heading")

In [Get Started with Distributed Training using XGBoost](../../train/getting-started-xgboost.html), we covered how to scale XGBoost single-model training with *Ray Train*.
For the rest of this tutorial, we will focus on how to optimize the hyperparameters of the XGBoost model using *Ray Tune*.

## [XGBoost Hyperparameters](#id8)[#](#xgboost-hyperparameters "Link to this heading")
...
### Maximum tree depth[#](#maximum-tree-depth "Link to this heading")
...
![Decision tree depth](../../_images/tune-xgboost-depth.svg)
...
of the tree.[#](#id2 "Link to this image")
...
### Minimum child weight[#](#minimum-child-weight "Link to this heading")
...
![Minimum child weight](../../_images/tune-xgboost-weight.svg)
...
to add leaves on the right side.[#](#id3 "Link to this image")
...
### Subsample size[#](#subsample-size "Link to this heading")
...
### Learning rate / Eta[#](#learning-rate-eta "Link to this heading")
...
\[
F\_m(x) = F\_{m-1}(x) + \eta \cdot \gamma\_{lm} \textbf{1}(x \in R\_{lm})
\]

This is just a complicated way to say that when we train we new decision tree,
represented by \(\gamma\_{lm} \textbf{1}(x \in R\_{lm})\), we want to dampen
its effect on the previous prediction \(F\_{m-1}(x)\) with a factor
\(\eta\).
...
### Number of boost rounds[#](#number-of-boost-rounds "Link to this heading")
...
## [Tuning the configuration parameters](#id9)[#](#tuning-the-configuration-parameters "Link to this heading")
...
| Trial name | status | loc | eta | max\_depth | min\_child\_weight | subsample | acc | iter | total time (s) |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| train\_breast\_cancer\_31c9f\_00000 | TERMINATED | 127.0.0.1:89735 | 0.0434196 | 8 | 1 | 0.530351 | 0.909091 | 1 | 0.0114911 |
| train\_breast\_cancer\_31c9f\_00001 | TERMINATED | 127.0.0.1:89734 | 0.0115669 | 6 | 2 | 0.996519 | 0.615385 | 1 | 0.01138 |
| train\_breast\_cancer\_31c9f\_00002 | TERMINATED | 127.0.0.1:89740 | 0.00124339 | 7 | 3 | 0.536078 | 0.629371 | 1 | 0.0096581 |
| train\_breast\_cancer\_31c9f\_00003 | TERMINATED | 127.0.0.1:89742 | 0.000400434 | 6 | 3 | 0.90014 | 0.601399 | 1 | 0.0103199 |
| train\_breast\_cancer\_31c9f\_00004 | TERMINATED | 127.0.0.1:89738 | 0.0121308 | 6 | 3 | 0.843156 | 0.629371 | 1 | 0.00843 |
| train\_breast\_cancer\_31c9f\_00005 | TERMINATED | 127.0.0.1:89733 | 0.0344144 | 2 | 3 | 0.513071 | 0.895105 | 1 | 0.00800109 |
| train\_breast\_cancer\_31c9f\_00006 | TERMINATED | 127.0.0.1:89737 | 0.0530037 | 7 | 2 | 0.920801 | 0.965035 | 1 | 0.0117419 |
| train\_breast\_cancer\_31c9f\_00007 | TERMINATED | 127.0.0.1:89741 | 0.000230442 | 3 | 3 | 0.946852 | 0.608392 | 1 | 0.00917387 |
| train\_breast\_cancer\_31c9f\_00008 | TERMINATED | 127.0.0.1:89739 | 0.00166323 | 4 | 1 | 0.588879 | 0.636364 | 1 | 0.011095 |
| train\_breast\_cancer\_31c9f\_00009 | TERMINATED | 127.0.0.1:89736 | 0.0753618 | 3 | 3 | 0.55103 | 0.909091 | 1 | 0.00776482 |
...
[the Tune docs](../api/search_space.html#tune-search-space).
...
## [Early stopping](#id10)[#](#early-stopping "Link to this heading")
...
described [in the Tune documentation](../api/schedulers.html#tune-schedulers).
...
[documentation](../api/schedulers.html#tune-scheduler-hyperband).
...
with [two XGBoost callbacks](../api/integration.html#tune-integration-xgboost)
...
Using AsyncHyperBand: num\_stopped=1  
...
| Trial name | status | loc | eta | max\_depth | min\_child\_weight | subsample | iter | total time (s) | eval-logloss | eval-error |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| train\_breast\_cancer\_32eb5\_00000 | TERMINATED | 127.0.0.1:89763 | 0.000830475 | 5 | 1 | 0.675899 | 10 | 0.0169384 | 0.640195 | 0.342657 |
...
[XGBoost Dynamic Resources Example](includes/xgboost_dynamic_resources_example.html).

## [Using fractional GPUs](#id11)[#](#using-fractional-gpus "Link to this heading")
...
## [Conclusion](#id12)[#](#conclusion "Link to this heading")
...
## [More XGBoost Examples](#id13)[#](#more-xgboost-examples "Link to this heading")
...
* [Get Started with Distributed Training using XGBoost](../../train/getting-started-xgboost.html): Shows how to scale XGBoost single-model training with *Ray Train* (as opposed to hyperparameter tuning with Ray Tune).

## [Learn More](#id14)[#](#learn-more "Link to this heading")
...
[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/tune/examples/tune-xgboost.ipynb)