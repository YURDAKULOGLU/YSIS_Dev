...
# Ray Tune FAQ[#](#ray-tune-faq "Link to this heading")

Here we try to answer questions that come up often.
If you still have questions after reading this FAQ, let us know!

## [What are Hyperparameters?](#id1)[#](#what-are-hyperparameters "Link to this heading")
...
![../_images/hyper-model-parameters.png](../_images/hyper-model-parameters.png)
...
![../_images/hyper-network-params.png](../_images/hyper-network-params.png)
...
## [Which search algorithm/scheduler should I choose?](#id2)[#](#which-search-algorithm-scheduler-should-i-choose "Link to this heading")

Ray Tune offers [many different search algorithms](api/suggestion.html#tune-search-alg)
and [schedulers](api/schedulers.html#tune-schedulers).
...
[ASHA for early termination of bad trials](api/schedulers.html#tune-scheduler-hyperband) (if your problem
supports early stopping).

**If your model is large**, you can try to either use
**Bayesian Optimization-based search algorithms** like [BayesOpt](api/suggestion.html#bayesopt)
to get good parameter configurations after few
trials. [Ax](api/suggestion.html#tune-ax) is similar but more robust to noisy data.
Please note that these algorithms only work well with **a small number of hyperparameters**.
Alternatively, you can use [Population Based Training](api/schedulers.html#tune-scheduler-pbt) which
...
work well. Take a look at [BOHB](api/schedulers.html#tune-scheduler-bohb) or [Optuna](api/suggestion.html#tune-optuna)
with the [ASHA](api/schedulers.html#tune-scheduler-hyperband) scheduler to combine the
...
or a TPE-based Bayesian Optimization algorithm such as [Optuna](api/suggestion.html#tune-optuna) or
[HyperOpt](api/suggestion.html#tune-hyperopt).

**Our go-to solution** is usually to use **random search** with
[ASHA for early stopping](api/schedulers.html#tune-scheduler-hyperband) for smaller problems.
Use [BOHB](api/schedulers.html#tune-scheduler-bohb) for **larger problems** with a **small number of hyperparameters**
and [Population Based Training](api/schedulers.html#tune-scheduler-pbt) for **larger problems** with a
**large number of hyperparameters** if a learning schedule is acceptable.

## [How do I choose hyperparameter ranges?](#id3)[#](#how-do-i-choose-hyperparameter-ranges "Link to this heading")
...
For instance, [XGBoost’s parameter overview](https://xgboost.readthedocs.io/en/latest/parameter.html)
...
## [How can I use nested/conditional search spaces?](#id4)[#](#how-can-i-use-nested-conditional-search-spaces "Link to this heading")
...
### Nested spaces[#](#nested-spaces "Link to this heading")
...
### Conditional spaces[#](#conditional-spaces "Link to this heading")

[Custom and conditional search spaces are explained in detail here](tutorials/tune-search-spaces.html#tune-custom-search).
...
### Conditional grid search[#](#conditional-grid-search "Link to this heading")
...
## [How does early termination (e.g. Hyperband/ASHA) work?](#id5)[#](#how-does-early-termination-e-g-hyperband-asha-work "Link to this heading")
...
## [Why are all my trials returning “1” iteration?](#id6)[#](#why-are-all-my-trials-returning-1-iteration "Link to this heading")
...
## [What are all these extra outputs?](#id7)[#](#what-are-all-these-extra-outputs "Link to this heading")
...
See the [How to use log metrics in Tune?](tutorials/tune-metrics.html#tune-autofilled-metrics) section for a glossary.

## [How do I set resources?](#id8)[#](#how-do-i-set-resources "Link to this heading")
...
a dict or a [`PlacementGroupFactory`](api/doc/ray.tune.execution.placement_groups.PlacementGroupFactory.html#ray.tune.execution.placement_groups.PlacementGroupFactory "ray.tune.execution.placement_groups.PlacementGroupFactory") object:
...
[placement groups](../ray-core/scheduling/placement-group.html#ray-placement-group-doc-ref) to request additional resources:
...
[placement groups documentation](../ray-core/scheduling/placement-group.html#ray-placement-group-doc-ref) to learn more
...
## [Why is my training stuck and Ray reporting that pending actor or tasks cannot be scheduled?](#id9)[#](#why-is-my-training-stuck-and-ray-reporting-that-pending-actor-or-tasks-cannot-be-scheduled "Link to this heading")
...
the trial using [placement groups](../ray-core/scheduling/placement-group.html#ray-placement-group-doc-ref), as outlined in
the section above.
...
## [How can I pass further parameter values to my trainable?](#id10)[#](#how-can-i-pass-further-parameter-values-to-my-trainable "Link to this heading")
...
just that, called [`tune.with_parameters()`](api/doc/ray.tune.with_parameters.html#ray.tune.with_parameters "ray.tune.with_parameters"):
...
[`tune.with_parameters()`](api/doc/ray.tune.with_parameters.html#ray.tune.with_parameters "ray.tune.with_parameters")
also works with class trainables. Please see
[`tune.with_parameters()`](api/doc/ray.tune.with_parameters.html#ray.tune.with_parameters "ray.tune.with_parameters") for more details and examples.

## [How can I reproduce experiments?](#id11)[#](#how-can-i-reproduce-experiments "Link to this heading")
...
## [How can I avoid bottlenecks?](#id12)[#](#how-can-i-avoid-bottlenecks "Link to this heading")
...
**Solution**: Use [`tune.with_parameters`](api/doc/ray.tune.with_parameters.html#ray.tune.with_parameters "ray.tune.with_parameters") to pass large objects to
...
**Solution**: You can use [cloud checkpointing](tutorials/tune-storage.html#tune-cloud-checkpointing) to save logs and checkpoints to a specified `storage_path`.
...
## [How can I develop and test Tune locally?](#id13)[#](#how-can-i-develop-and-test-tune-locally "Link to this heading")

First, follow the instructions in [Building Ray (Python Only)](../ray-contribute/development.html#python-develop) to develop Tune without compiling Ray.
...
## [How can I get started contributing to Tune?](#id14)[#](#how-can-i-get-started-contributing-to-tune "Link to this heading")

We use GitHub to track issues, feature requests, and bugs. Take a look at the
ones labeled [“good first issue”](https://github.com/ray-project/ray/issues?utf8=%E2%9C%93&q=is%3Aissue+is%3Aopen+label%3A%22good-first-issue%22) and [“help wanted”](https://github.com/ray-project/ray/issues?q=is%3Aopen+is%3Aissue+label%3A%22help-wanted%22) for a place to start.
...
## [How can I make my Tune experiments reproducible?](#id15)[#](#how-can-i-make-my-tune-experiments-reproducible "Link to this heading")
...
## [How can I use large datasets in Tune?](#id16)[#](#how-can-i-use-large-datasets-in-tune "Link to this heading")
...
Objects passed with this wrapper will be stored on the [Ray object store](../ray-core/objects.html#objects-in-ray) and will
be automatically fetched and passed to your trainable as a parameter.

Tip

If the objects are small in size or already exist in the [Ray Object Store](../ray-core/objects.html#objects-in-ray), there’s no need to use `tune.with_parameters()`. You can use [partials](https://docs.python.org/3/library/functools.html#functools.partial) or pass in directly to `config` instead.
...
## [How can I upload my Tune results to cloud storage?](#id17)[#](#how-can-i-upload-my-tune-results-to-cloud-storage "Link to this heading")

See [Configuring Tune with cloud storage (AWS S3, Google Cloud Storage)](tutorials/tune-storage.html#tune-cloud-checkpointing).
...
Please [see here for more tips](../cluster/vms/user-guides/launching-clusters/aws.html#aws-cluster-s3).

## [How can I use Tune with Kubernetes?](#id18)[#](#how-can-i-use-tune-with-kubernetes "Link to this heading")

You should configure shared storage. See this user guide: [How to Configure Persistent Storage in Ray Tune](tutorials/tune-storage.html#tune-storage-options).

## [How can I use Tune with Docker?](#id19)[#](#how-can-i-use-tune-with-docker "Link to this heading")

You should configure shared storage. See this user guide: [How to Configure Persistent Storage in Ray Tune](tutorials/tune-storage.html#tune-storage-options).

## [How do I configure search spaces?](#id20)[#](#how-do-i-configure-search-spaces "Link to this heading")
...
Read about this in the [Search Space API](api/search_space.html#tune-search-space) page.

## [How do I access relative filepaths in my Tune training function?](#id21)[#](#how-do-i-access-relative-filepaths-in-my-tune-training-function "Link to this heading")
...
[`tune.get_context().get_trial_dir()`](api/doc/ray.tune.TuneContext.get_trial_dir.html#ray.tune.TuneContext.get_trial_dir "ray.tune.TuneContext.get_trial_dir")
...
## [How can I run multiple Ray Tune jobs on the same cluster at the same time (multi-tenancy)?](#id22)[#](#how-can-i-run-multiple-ray-tune-jobs-on-the-same-cluster-at-the-same-time-multi-tenancy "Link to this heading")
...
## [How can I continue training a completed Tune experiment for longer and with new configurations (iterative experimentation)?](#id23)[#](#how-can-i-continue-training-a-completed-tune-experiment-for-longer-and-with-new-configurations-iterative-experimentation "Link to this heading")
...
[How to Enable Fault Tolerance in Ray Tune](tutorials/tune-fault-tolerance.html#tune-fault-tolerance-ref) explains that the usage of [`Tuner.restore`](api/doc/ray.tune.Tuner.restore.html#ray.tune.Tuner.restore "ray.tune.Tuner.restore")
...
[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/tune/faq.rst)