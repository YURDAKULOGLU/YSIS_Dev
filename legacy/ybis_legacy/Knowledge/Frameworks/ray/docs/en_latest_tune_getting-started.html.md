Getting Started with Ray Tune — Ray 2.53.0
...
# Getting Started with Ray Tune[#](#getting-started-with-ray-tune "Link to this heading")
...
## Setting Up a PyTorch Model to Tune[#](#setting-up-a-pytorch-model-to-tune "Link to this heading")
...
## Setting up a `Tuner` for a Training Run with Tune[#](#setting-up-a-tuner-for-a-training-run-with-tune "Link to this heading")

Below, we define a function that trains the PyTorch model for multiple epochs.
This function will be executed on a separate [Ray Actor (process)](../ray-core/actors.html#actor-guide) underneath the hood,
so we need to communicate the performance of the model back to Tune (which is on the main Python process).

To do this, we call [`tune.report()`](api/doc/ray.tune.report.html#ray.tune.report "ray.tune.report") in our training function,
which sends the performance value back to Tune. Since the function is executed on the separate process,
make sure that the function is [serializable by Ray](../ray-core/objects/serialization.html#serialization-guide).
...
Let’s run one trial by calling [Tuner.fit](api/execution.html#tune-run-ref) and [randomly sample](api/search_space.html#tune-search-space)
...
`Tuner.fit` returns an [ResultGrid object](api/result_grid.html#tune-analysis-docs).
...
To limit the number of concurrent trials, use the [ConcurrencyLimiter](api/suggestion.html#limiter).

## Early Stopping with Adaptive Successive Halving (ASHAScheduler)[#](#early-stopping-with-adaptive-successive-halving-ashascheduler "Link to this heading")

Let’s integrate early stopping into our optimization process. Let’s use [ASHA](api/schedulers.html#tune-scheduler-hyperband), a scalable algorithm for [principled early stopping](https://blog.ml.cmu.edu/2018/12/12/massively-parallel-hyperparameter-optimization/).
...
See [the TrialScheduler documentation](api/schedulers.html#tune-schedulers) for more details of available schedulers and library integrations.
...
[![../_images/tune-df-plot.png](../_images/tune-df-plot.png)](../_images/tune-df-plot.png)

You can also use [TensorBoard](tutorials/tune-output.html#tensorboard) for visualizing results.

```
$ tensorboard --logdir {logdir}
```

## Using Search Algorithms in Tune[#](#using-search-algorithms-in-tune "Link to this heading")

In addition to [TrialSchedulers](api/schedulers.html#tune-schedulers), you can further optimize your hyperparameters
by using an intelligent search technique like Bayesian Optimization.
To do this, you can use a Tune [Search Algorithm](api/suggestion.html#tune-search-alg).
...
Tune allows you to use some search algorithms in combination with different trial schedulers. See [this page for more details](api/schedulers.html#tune-schedulers).

## Evaluating Your Model after Tuning[#](#evaluating-your-model-after-tuning "Link to this heading")

You can evaluate the best trained model using the [ExperimentAnalysis object](api/result_grid.html#tune-analysis-docs) to retrieve the best model:
...
* Check out the [Tune tutorials](tutorials/overview.html#tune-guides) for guides on using Tune with your preferred machine learning library.
* Browse our [gallery of examples](examples/index.html#tune-examples-others) to see how to use Tune with PyTorch, XGBoost, Tensorflow, etc.
...
* To check how your application is doing, you can use the [Ray dashboard](../ray-observability/getting-started.html#observability-getting-started).

On this page

[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/tune/getting-started.rst)