Running Distributed Experiments with Ray Tune — Ray 2.53.0
...
# Running Distributed Experiments with Ray Tune[#](#running-distributed-experiments-with-ray-tune "Link to this heading")

Tune is commonly used for large-scale distributed hyperparameter optimization. This page will overview how to setup and launch a distributed experiment along with [commonly used commands](#tune-distributed-common) for Tune when running distributed experiments.

## Summary[#](#summary "Link to this heading")

To run a distributed experiment with Tune, you need to:

1. First, [start a Ray cluster](../../cluster/getting-started.html#cluster-index) if you have not already.
2. Run the script on the head node, or use [ray submit](../../cluster/cli.html#ray-submit-doc), or use [Ray Job Submission](../../cluster/running-applications/job-submission/index.html#jobs-overview).

## Example: Distributed Tune on AWS VMs[#](#example-distributed-tune-on-aws-vms "Link to this heading")

Follow the instructions below to launch nodes on AWS (using the Deep Learning AMI). See the [cluster setup documentation](../../cluster/getting-started.html#cluster-index). Save the below cluster configuration (`tune-default.yaml`):
...
[![../../_images/tune-upload.png](../../_images/tune-upload.png)](../../_images/tune-upload.png)
...
Note that you can customize the directory of results by specifying: `RunConfig(storage_path=..)`, taken in by `Tuner`. You can then point TensorBoard to that directory to visualize results. You can also use [awless](https://github.com/wallix/awless) for easy cluster management on AWS.

## Running a Distributed Tune Experiment[#](#running-a-distributed-tune-experiment "Link to this heading")
...
## Storage Options in a Distributed Tune Run[#](#storage-options-in-a-distributed-tune-run "Link to this heading")

In a distributed experiment, you should try to use [cloud checkpointing](tune-storage.html#tune-cloud-checkpointing) to
reduce synchronization overhead. For this, you just have to specify a remote `storage_path` in the
[`RunConfig`](../api/doc/ray.tune.RunConfig.html#ray.tune.RunConfig "ray.tune.RunConfig").

`my_trainable` is a user-defined [Tune Trainable](../key-concepts.html#tune-60-seconds-trainables) in the following example:
...
[guide on configuring storage in a distributed Tune experiment](tune-storage.html#tune-storage-options).

## Tune Runs on preemptible instances[#](#tune-runs-on-preemptible-instances "Link to this heading")
...
[checkpointing](tune-trial-checkpoints.html#tune-trial-checkpoint).
...
### Example for Using Tune with Spot instances (AWS)[#](#example-for-using-tune-with-spot-instances-aws "Link to this heading")
...
1. Download a full example Tune experiment script here. This includes a Trainable with checkpointing: [`mnist_pytorch_trainable.py`](../../_downloads/8515735d7ae45b272ee9ef78046d863d/mnist_pytorch_trainable.py). To run this example, you will need to install the following:

```
$ pip install ray torch torchvision filelock
```

2. Download an example cluster yaml here: [`tune-default.yaml`](../../_downloads/6adeb623647a7151ee6b7b6553dda299/tune-default.yaml)
...
You should see Tune eventually continue the trials on a different worker node. See the [Fault Tolerance](#tune-fault-tol) section for more details.
...
## Fault Tolerance of Tune Runs[#](#fault-tolerance-of-tune-runs "Link to this heading")
...
[some form of persistent storage is configured to access the trial’s checkpoint](tune-storage.html#tune-storage-options)),
...
See [here for information on checkpointing](tune-trial-checkpoints.html#tune-trial-checkpoint).
...
### Recovering From Failures[#](#recovering-from-failures "Link to this heading")

Tune automatically persists the progress of your entire experiment (a `Tuner.fit()` session), so if an experiment crashes or is otherwise cancelled, it can be resumed through [`restore()`](../api/doc/ray.tune.Tuner.restore.html#ray.tune.Tuner.restore "ray.tune.Tuner.restore").

## Common Tune Commands[#](#common-tune-commands "Link to this heading")

Below are some commonly used commands for submitting experiments. Please see the [Clusters page](../../cluster/getting-started.html#cluster-index) to see find more comprehensive documentation of commands.
...
[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/tune/tutorials/tune-distributed.rst)