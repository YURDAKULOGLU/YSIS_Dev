A Guide To Parallelism and Resources for Ray Tune — Ray 2.53.0
...
# A Guide To Parallelism and Resources for Ray Tune[#](#a-guide-to-parallelism-and-resources-for-ray-tune "Link to this heading")
...
You can override this per trial resources with [`tune.with_resources`](../api/doc/ray.tune.with_resources.html#ray.tune.with_resources "ray.tune.with_resources"). Here you can
specify your resource requests using either a dictionary or a
[`PlacementGroupFactory`](../api/doc/ray.tune.execution.placement_groups.PlacementGroupFactory.html#ray.tune.execution.placement_groups.PlacementGroupFactory "ray.tune.execution.placement_groups.PlacementGroupFactory")
...
[autoscaling behavior](../../cluster/getting-started.html#cluster-index) if you’re using the Ray cluster launcher.
...
See the [`PlacementGroupFactory documentation`](../api/doc/ray.tune.execution.placement_groups.PlacementGroupFactory.html#ray.tune.execution.placement_groups.PlacementGroupFactory "ray.tune.execution.placement_groups.PlacementGroupFactory")
...
## How to leverage GPUs in Tune?[#](#how-to-leverage-gpus-in-tune "Link to this heading")
...
You can find an example of this in the [Keras MNIST example](../examples/tune_mnist_keras.html).
...
you can use [`tune.utils.wait_for_gpu`](../api/doc/ray.tune.utils.wait_for_gpu.html#ray.tune.utils.wait_for_gpu "ray.tune.utils.wait_for_gpu").

## How to run distributed training with Tune?[#](#how-to-run-distributed-training-with-tune "Link to this heading")
...
For more details, see [Ray Train Hyperparameter Optimization](../../train/user-guides/hyperparameter-optimization.html#train-tune).

## How to limit concurrency in Tune?[#](#how-to-limit-concurrency-in-tune "Link to this heading")

To specifies the max number of trials to run concurrently, set `max_concurrent_trials` in [`TuneConfig`](../api/doc/ray.tune.TuneConfig.html#ray.tune.TuneConfig "ray.tune.tune_config.TuneConfig").
...
[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/tune/tutorials/tune-resources.rst)