How to Configure Persistent Storage in Ray Tune — Ray 2.53.0
...
# How to Configure Persistent Storage in Ray Tune[#](#how-to-configure-persistent-storage-in-ray-tune "Link to this heading")

See also

Before diving into storage options, one can take a look at
[the different types of data stored by Tune](tune-trial-checkpoints.html#tune-persisted-experiment-data).
...
## Storage Options in Tune[#](#storage-options-in-tune "Link to this heading")
...
See [`SyncConfig`](../api/doc/ray.tune.SyncConfig.html#ray.tune.SyncConfig "ray.tune.SyncConfig") for the full set of configuration options as well as more details.

### Configuring Tune with cloud storage (AWS S3, Google Cloud Storage)[#](#configuring-tune-with-cloud-storage-aws-s3-google-cloud-storage "Link to this heading")
...
URI, rather than the local experiment directory on the head node. See [here for an example](#tune-syncing-restore-from-uri).

### Configuring Tune with a network filesystem (NFS)[#](#configuring-tune-with-a-network-filesystem-nfs "Link to this heading")
...
### Configure Tune without external persistent storage[#](#configure-tune-without-external-persistent-storage "Link to this heading")

#### On a single-node cluster[#](#on-a-single-node-cluster "Link to this heading")
...
unless you customize this with `storage_path` and `name` in [`RunConfig`](../api/doc/ray.tune.RunConfig.html#ray.tune.RunConfig "ray.tune.RunConfig").
...
#### On a multi-node cluster (Deprecated)[#](#on-a-multi-node-cluster-deprecated "Link to this heading")
...
See [How to Enable Fault Tolerance in Ray Tune](tune-fault-tolerance.html#tune-fault-tolerance-ref) for more information on resuming experiments.
...
See [Appendix: Types of data stored by Tune](tune-trial-checkpoints.html#tune-persisted-experiment-data) for an overview of other experiment data that Tune needs to persist.

### Example: Running Tune with cloud storage[#](#example-running-tune-with-cloud-storage "Link to this heading")

Let’s assume that you’re running this example script from your Ray cluster’s head node.

In the example below, `my_trainable` is a Tune [trainable](../api/trainable.html#trainable-docs)
...
[`restore()`](../api/doc/ray.tune.Tuner.restore.html#ray.tune.Tuner.restore "ray.tune.Tuner.restore") for more details.

## Advanced configuration[#](#advanced-configuration "Link to this heading")

See [Ray Train’s section on advanced storage configuration](../../train/user-guides/persistent-storage.html#train-storage-advanced).
All of the configurations also apply to Ray Tune.

On this page

[Edit
on GitHub](https://github.com/ray-project/ray/edit/master/doc/source/tune/tutorials/tune-storage.rst)