...
the [migration guide](../migration/) for more.
...
Neovim 0.10 (with [`nvim-lspconfig`](https://github.com/neovim/nvim-lspconfig))Neovim 0.11+ (with [`vim.lsp.config`](https://neovim.io/doc/user/lsp.html#vim.lsp.config()))
...
language server for certain capabilities, like [`textDocument/hover`](../features/#hover):
...
[`logLevel`](../settings/#loglevel) setting:
...
file with the [`logFile`](../settings/#logfile) setting.
...
With the [ALE](https://github.com/dense-analysis/ale) plugin for Neovim or Vim.
*Neovim (using Lua):*
...
like [`textDocument/hover`](../features/#hover) by adding the following to the function `s:on_lsp_buffer_enabled()`:
...
Ruff can also be integrated via [efm language server](https://github.com/mattn/efm-langserver) in just a few lines.
...
use Ruff alongside another language server. Note that this setup won't use the [server settings](../settings/)
...
To set up editor-wide Ruff options, provide the [server settings](../settings/)
