...
[Skip to content](#the-ruff-linter)
...
The set of enabled rules is controlled via the [`lint.select`](../settings/#lint_select),
[`lint.extend-select`](../settings/#lint_extend-select), and [`lint.ignore`](../settings/#lint_ignore) settings.
...
Rule selectors like [`lint.select`](../settings/#lint_select) and [`lint.ignore`](../settings/#lint_ignore) accept either
...
of `F401`. For more on configuring Ruff via `pyproject.toml`, see [*Configuring Ruff*](../configuration/).
...
To resolve the enabled rule set, Ruff may need to reconcile [`lint.select`](../settings/#lint_select) and
[`lint.ignore`](../settings/#lint_ignore) from a variety of sources, including the current `pyproject.toml`,
any inherited `pyproject.toml` files, and the CLI (e.g., [`--select`](../settings/#lint_select)).

In those scenarios, Ruff uses the "highest-priority" [`select`](../settings/#lint_select) as the basis for
the rule set, and then applies [`extend-select`](../settings/#lint_extend-select) and
[`ignore`](../settings/#lint_ignore) adjustments. CLI options are given higher priority than
...
whether a rule supports fixing, see [*Rules*](../rules/).
...
For example, [`unnecessary-iterable-allocation-for-first-element`](../rules/unnecessary-iterable-allocation-for-first-element/)
...
by setting the [`unsafe-fixes`](../settings/#unsafe-fixes) setting to `false` or using the `--no-unsafe-fixes` flag.
...
To limit the set of rules that Ruff should fix, use the [`lint.fixable`](../settings/#lint_fixable)
or [`lint.extend-fixable`](../settings/#lint_extend-fixable), and [`lint.unfixable`](../settings/#lint_unfixable) settings.

For example, the following configuration would enable fixes for all rules except
[`unused-imports`](../rules/unused-import/) (`F401`):
...
To omit a lint rule everywhere, add it to the "ignore" list via the [`lint.ignore`](../settings/#lint_ignore)
...
see the [`lint.per-file-ignores`](../settings/#lint_per-file-ignores) setting.
...
they *say* they ignore are actually being triggered and suppressed. To flag
...
Ruff can also *remove* any unused suppression comments via its fix functionality.
...
* `--exit-non-zero-on-fix` will cause Ruff to exit with a status code of `1` if violations were
  found, *even if* all such violations were fixed automatically. Note that the use of
