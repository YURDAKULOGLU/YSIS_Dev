...
* Validators and Event History:
  ...
  + When a Validator throws an error, the Update is rejected, the Update is not run, and `WorkflowExecutionUpdateAccepted` *won't* be added to the Event History.
...
Use [`Workflow.newExternalWorkflowStub`](https://javadoc.io/doc/io.temporal/temporal-sdk/latest/io/temporal/workflow/Workflow.html#newExternalWorkflowStub(java.lang.Class,io.temporal.api.common.v1.WorkflowExecution)) in your *current* Workflow to create an `ExternalWorkflowStub` for the other Workflow.
...
### Do blocking operations in handlers[​](#blocking-handlers "Direct link to Do blocking operations in handlers")
...
### Add blocking wait conditions[​](#block-with-wait "Direct link to Add blocking wait conditions")
...
#### Wait for conditions in handlers[​](#wait-in-handlers "Direct link to Wait for conditions in handlers")
...
### Use `@WorkflowInit` to operate on Workflow input before any handler executes[​](#use-workflowinit-to-operate-on-workflow-input-before-any-handler-executes "Direct link to use-workflowinit-to-operate-on-workflow-input-before-any-handler-executes")
...
### Use locks to prevent concurrent handler execution[​](#control-handler-concurrency "Direct link to Use locks to prevent concurrent handler execution")
...
### How to set a Dynamic Activity[​](#set-a-dynamic-activity "Direct link to How to set a Dynamic Activity")
...
### How to set a Dynamic Signal[​](#set-a-dynamic-signal "Direct link to How to set a Dynamic Signal")
...
### How to set a Dynamic Query[​](#set-a-dynamic-query "Direct link to How to set a Dynamic Query")
...
### How to set a Dynamic Update[​](#set-a-dynamic-update "Direct link to How to set a Dynamic Update")
...
* [Write message handlers](#writing-message-handlers)
  + [Query handlers](#queries)+ [Signal handlers](#signals)+ [Update handlers and validators](#updates)* [Send messages](#send-messages)
    + [Send a Query](#send-query)+ [Send a Signal](#send-signal)+ [Send an Update](#send-update-from-client)* [Message handler patterns](#message-handler-patterns)
      + [Do blocking operations in handlers](#blocking-handlers)+ [Add blocking wait conditions](#block-with-wait)+ [Use `@WorkflowInit` to operate on Workflow input before any handler executes](#use-workflowinit-to-operate-on-workflow-input-before-any-handler-executes)+ [Use locks to prevent concurrent handler execution](#control-handler-concurrency)* [Message handler troubleshooting](#message-handler-troubleshooting)
        + [Problems when sending a Signal](#signal-problems)+ [Problems when sending an Update](#update-problems)+ [Problems when sending a Query](#query-problems)* [Dynamic components](#dynamic-handler)
          + [Set a Dynamic Workflow](#set-a-dynamic-workflow)+ [How to set a Dynamic Activity](#set-a-dynamic-activity)+ [How to set a Dynamic Signal](#set-a-dynamic-signal)+ [How to set a Dynamic Query](#set-a-dynamic-query)+ [How to set a Dynamic Update](#set-a-dynamic-update)