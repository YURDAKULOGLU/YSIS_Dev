Workflow message passing - Python SDK | Temporal Platform Documentation
...
## Write message handlers[​](#writing-message-handlers "Direct link to Write message handlers")
...
### Query handlers[​](#queries "Direct link to Query handlers")
...
### Signal handlers[​](#signals "Direct link to Signal handlers")
...
### Update handlers and validators[​](#updates "Direct link to Update handlers and validators")
...
* Validators and Event History:
  ...
  + When a Validator raises an error, the Update is rejected and `WorkflowExecutionUpdateAccepted` *won't* be added to the Event History.
...
* Use [start\_workflow](https://python.temporal.io/temporalio.client.Client.html#start_workflow) to start a Workflow and return its handle.
* Use [get\_workflow\_handle\_for](https://python.temporal.io/temporalio.client.Client.html#get_workflow_handle_for) to retrieve a typed Workflow handle by its Workflow Id.
...
### Send a Query[​](#send-query "Direct link to Send a Query")
...
### Send a Signal[​](#send-signal "Direct link to Send a Signal")
...
#### Send a Signal from a Client[​](#send-signal-from-client "Direct link to Send a Signal from a Client")
...
#### Send a Signal from a Workflow[​](#send-signal-from-workflow "Direct link to Send a Signal from a Workflow")
...
[See full sample](https://github.com/temporalio/documentation/blob/main/sample-apps/python/signal_your_workflow/signal_external_wf_dacx.py)
...
#### Signal-With-Start[​](#signal-with-start "Direct link to Signal-With-Start")
...
[See full sample](https://github.com/temporalio/documentation/blob/main/sample-apps/python/signal_your_workflow/signal_with_start_dacx.py)
...
### Send an Update[​](#send-update-from-client "Direct link to Send an Update")
...
#### Update-With-Start[​](#update-with-start "Direct link to Update-With-Start")
...
Here's an example taken from the [lazy\_initialization](https://github.com/temporalio/samples-python/blob/main/message_passing/update_with_start/lazy_initialization/starter.py) sample:
...
## Message handler patterns[​](#message-handler-patterns "Direct link to Message handler patterns")
...
### Use async handlers[​](#async-handlers "Direct link to Use async handlers")
...
### Add wait conditions to block[​](#add-wait-conditions-to-block "Direct link to Add wait conditions to block")
...
#### Wait for a Signal or Update to arrive[​](#wait-for-a-signal-or-update-to-arrive "Direct link to Wait for a Signal or Update to arrive")
...
#### Use wait conditions in handlers[​](#wait-in-message-handler "Direct link to Use wait conditions in handlers")
...
#### Ensure your handlers finish before the Workflow completes[​](#wait-for-message-handlers "Direct link to Ensure your handlers finish before the Workflow completes")
...
### Use `@workflow.init` to operate on Workflow input before any handler executes[​](#use-workflowinit-to-operate-on-workflow-input-before-any-handler-executes "Direct link to use-workflowinit-to-operate-on-workflow-input-before-any-handler-executes")
...
### Use `asyncio.Lock` to prevent concurrent handler execution[​](#control-handler-concurrency "Direct link to control-handler-concurrency")
...
## Message handler troubleshooting[​](#message-handler-troubleshooting "Direct link to Message handler troubleshooting")
...
### Problems when sending a Signal[​](#signal-problems "Direct link to Problems when sending a Signal")
...
### Problems when sending an Update[​](#update-problems "Direct link to Problems when sending an Update")
...
* **Update failed**: You'll receive a [`temporalio.client.WorkflowUpdateFailedError`](https://python.temporal.io/temporalio.client.WorkflowUpdateFailedError.html) exception.
  ...
  + Any error listed in [workflow\_failure\_exception\_types](https://python.temporal.io/temporalio.worker.Worker.html) (empty by default)
...
### Problems when sending a Query[​](#query-problems "Direct link to Problems when sending a Query")
...
## Dynamic components[​](#dynamic-handler "Direct link to Dynamic components")
...
### Set a dynamic Signal, Query, or Update handler[​](#set-a-dynamic-signal "Direct link to Set a dynamic Signal, Query, or Update handler")
...
Use a [payload\_converter](https://python.temporal.io/temporalio.workflow.html#payload_converter) function to convert `RawValue` objects to your required type.
For example:
...
[See full sample](https://github.com/temporalio/documentation/blob/main/sample-apps/python/dynamic_handlers/your_dynamic_workflow_dacx.py)
...
Use a [payload\_converter](https://python.temporal.io/temporalio.activity.html#payload_converter) function to convert `RawValue` objects to your required types.
For example:

[See full sample](https://github.com/temporalio/documentation/blob/main/sample-apps/python/dynamic_handlers/your_dynamic_activity_dacx.py)
...
* [Write message handlers](#writing-message-handlers)
  + [Query handlers](#queries)+ [Signal handlers](#signals)+ [Update handlers and validators](#updates)* [Send messages](#send-messages)
    + [Send a Query](#send-query)+ [Send a Signal](#send-signal)+ [Send an Update](#send-update-from-client)* [Message handler patterns](#message-handler-patterns)
      + [Use async handlers](#async-handlers)+ [Add wait conditions to block](#add-wait-conditions-to-block)+ [Use `@workflow.init` to operate on Workflow input before any handler executes](#use-workflowinit-to-operate-on-workflow-input-before-any-handler-executes)+ [Use `asyncio.Lock` to prevent concurrent handler execution](#control-handler-concurrency)* [Message handler troubleshooting](#message-handler-troubleshooting)
        + [Problems when sending a Signal](#signal-problems)+ [Problems when sending an Update](#update-problems)+ [Problems when sending a Query](#query-problems)* [Dynamic components](#dynamic-handler)
          + [Set a dynamic Signal, Query, or Update handler](#set-a-dynamic-signal)+ [Set a dynamic Workflow](#set-a-dynamic-workflow)+ [Set a dynamic Activity](#set-a-dynamic-activity)