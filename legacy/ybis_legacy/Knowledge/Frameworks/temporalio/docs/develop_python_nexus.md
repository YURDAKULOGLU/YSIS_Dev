Temporal Nexus - Python SDK Feature Guide | Temporal Platform Documentation
...
[hello\_nexus/service.py](https://github.com/temporalio/samples-python/blob/main/hello_nexus/service.py)
...
## Develop a Nexus Service handler and Operation handlers[​](#develop-nexus-service-operation-handlers "Direct link to Develop a Nexus Service handler and Operation handlers")
...
[hello\_nexus/handler/service\_handler.py](https://github.com/temporalio/samples-python/blob/main/hello_nexus/handler/service_handler.py)
...
The [nexus\_sync\_operations](https://github.com/temporalio/samples-python/blob/main/nexus_sync_operations) sample shows how to create a Nexus Service uses synchronous operations to send Updates and Queries:

[nexus\_sync\_operations/handler/service\_handler.py](https://github.com/temporalio/samples-python/blob/main/nexus_sync_operations/handler/service_handler.py)
...
[nexus\_multiple\_args/handler/service\_handler.py](https://github.com/temporalio/samples-python/blob/main/nexus_multiple_args/handler/service_handler.py)
...
### Register your Nexus Service handler in a Worker[​](#register-your-nexus-service-handler-in-a-worker "Direct link to Register your Nexus Service handler in a Worker")
...
[hello\_nexus/handler/worker.py](https://github.com/temporalio/samples-python/blob/main/hello_nexus/handler/worker.py)
...
[hello\_nexus/caller/workflows.py](https://github.com/temporalio/samples-python/blob/main/hello_nexus/caller/workflows.py)
...
### Register the caller Workflow in a Worker and start the caller Workflow[​](#register-the-caller-workflow-in-a-worker-and-start-the-caller-workflow "Direct link to Register the caller Workflow in a Worker and start the caller Workflow")
...
See [hello\_nexus/caller/app.py](https://github.com/temporalio/samples-python/blob/main/hello_nexus/caller/app.py) for reference.

## Exceptions in Nexus operations[​](#exceptions-in-nexus-operations "Direct link to Exceptions in Nexus operations")
...
* [Run the Temporal Development Server with Nexus enabled](#run-the-temporal-nexus-development-server)* [Create caller and handler Namespaces](#create-caller-handler-namespaces)* [Create a Nexus Endpoint to route requests from caller to handler](#create-nexus-endpoint)* [Define the Nexus Service contract](#define-nexus-service-contract)* [Develop a Nexus Service handler and Operation handlers](#develop-nexus-service-operation-handlers)
          + [Develop a Synchronous Nexus Operation handler](#develop-a-synchronous-nexus-operation-handler)+ [Develop an Asynchronous Nexus Operation handler to start a Workflow](#develop-an-asynchronous-nexus-operation-handler-to-start-a-workflow)
              - [Map a Nexus Operation input to multiple Workflow arguments](#map-a-nexus-operation-input-to-multiple-workflow-arguments)+ [Register your Nexus Service handler in a Worker](#register-your-nexus-service-handler-in-a-worker)* [Develop a caller Workflow that uses the Nexus Service](#develop-caller-workflow-nexus-service)
            + [Register the caller Workflow in a Worker and start the caller Workflow](#register-the-caller-workflow-in-a-worker-and-start-the-caller-workflow)* [Exceptions in Nexus operations](#exceptions-in-nexus-operations)* [Canceling a Nexus Operation](#canceling-a-nexus-operation)* [Make Nexus calls across Namespaces in Temporal Cloud](#nexus-calls-across-namespaces-temporal-cloud)
                  + [Install the latest `tcld` CLI and generate certificates](#install-the-latest-tcld-cli-and-generate-certificates)+ [Create caller and handler Namespaces](#create-caller-and-handler-namespaces)+ [Create a Nexus Endpoint to route requests from caller to handler](#create-a-nexus-endpoint-to-route-requests-from-caller-to-handler)* [Observability](#observability)
