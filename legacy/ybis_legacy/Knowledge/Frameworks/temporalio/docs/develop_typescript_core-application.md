Core application - TypeScript SDK | Temporal Platform Documentation
...
### How to find the TypeScript SDK API reference[​](#api-reference "Direct link to How to find the TypeScript SDK API reference")
...
### How to import an ECMAScript module[​](#ecmascript-modules "Direct link to How to import an ECMAScript module")
...
## Linting and types in TypeScript[​](#linting-and-types "Direct link to Linting and types in TypeScript")
...
## How to connect a Temporal Client to a Temporal Service[​](#connect-to-a-dev-cluster "Direct link to How to connect a Temporal Client to a Temporal Service")
...
## How to connect to Temporal Cloud[​](#connect-to-temporal-cloud "Direct link to How to connect to Temporal Cloud")
...
### How to customize your Workflow Type[​](#workflow-type "Direct link to How to customize your Workflow Type")
...
## How to develop a basic Activity[​](#develop-activities "Direct link to How to develop a basic Activity")
...
### How to customize your Activity Type[​](#activity-type "Direct link to How to customize your Activity Type")
...
### Important design patterns for Activities[​](#activity-design-patterns "Direct link to Important design patterns for Activities")
...
#### Share dependencies in Activity functions (dependency injection)[​](#share-dependencies-in-activity-functions-dependency-injection "Direct link to Share dependencies in Activity functions (dependency injection)")
...
#### Import multiple Activities simultaneously[​](#import-multiple-activities-simultaneously "Direct link to Import multiple Activities simultaneously")
...
#### Dynamically reference Activities[​](#dynamically-reference-activities "Direct link to Dynamically reference Activities")
...
In TypeScript, you never call an Activity function directly. Instead, you pass in the *types* of your Activities and Activity options to the `proxyActivities` function. This will give you an *Activity Handle*, a type-safe proxy object with the same function names and signatures as your real activities. From the Activity Handle, you can call your Activities as if they were normal async functions.
...
When you call a proxied function, the Workflow does not execute the Activity code directly. Instead, it schedules an Activity Task. After the Activity Task is scheduled, it becomes available for a Worker to pick up and execute.
This results in the set of three [Activity Task](/tasks#activity-task) related Events: [ActivityTaskScheduled](/references/events#activitytaskscheduled), [ActivityTaskStarted](/references/events#activitytaskstarted), and [ActivityTaskCompleted](/references/events#activitytaskcompleted) in your Workflow Execution Event History.

The Worker may run many Activity executions at the same time, all using the same Activity function code. Temporal can also retry an Activity if it fails or times out.
For this reason, you should write Activities to be [idempotent](/activity-definition#idempotency): calling them multiple times with the same input should have the same effect as calling them once.

tip

Every Activity call you make is recorded in the Workflow’s execution history, including the parameters you pass in and the value that comes back. This history is what allows Temporal to recover a Workflow after a failure.
Because the entire history must be stored and replayed, avoid passing large objects as Activity inputs or return values. Keeping payloads small will help your Workflows replay and recover efficiently.
...
## How to run Worker Processes[​](#run-a-dev-worker "Direct link to How to run Worker Processes")
...
## How to run a Worker on Docker in TypeScript[​](#run-a-worker-on-docker "Direct link to How to run a Worker on Docker in TypeScript")
...
We recommend an LTS Node.js release such as 18, 20, 22, or 24.
...
### Using `node:slim` images[​](#using-nodeslim-images "Direct link to using-nodeslim-images")
...
### Using `distroless/nodejs` images[​](#using-distrolessnodejs-images "Direct link to using-distrolessnodejs-images")
...
However, some tools required for the build process (notably the `npm` command) are *not* included in the `distroless/nodejs` image.
...
### Properly configure Node.js memory in Docker[​](#properly-configure-nodejs-memory-in-docker "Direct link to Properly configure Node.js memory in Docker")

By default, `node` configures its maximum old-gen memory to 25% of the *physical memory* of the machine on which it is executing, with a maximum of 4 GB.
...
### Do not use Alpine[​](#do-not-use-alpine "Direct link to Do not use Alpine")
...
## How to shut down a Worker and track its state[​](#shut-down-a-worker "Direct link to How to shut down a Worker and track its state")
...
## How to start a Workflow Execution[​](#start-workflow-execution "Direct link to How to start a Workflow Execution")
...
### How to set a Workflow's Task Queue[​](#set-task-queue "Direct link to How to set a Workflow's Task Queue")
...
### How to set a Workflow Id[​](#workflow-id "Direct link to How to set a Workflow Id")
...
### How to get the results of a Workflow Execution[​](#get-workflow-results "Direct link to How to get the results of a Workflow Execution")
...
## Cancellation scopes in Typescript[​](#cancellation-scopes "Direct link to Cancellation scopes in Typescript")
...
### CancelledFailure[​](#cancelledfailure "Direct link to CancelledFailure")
...
### Internal cancellation example[​](#internal-cancellation-example "Direct link to Internal cancellation example")
...
### External cancellation example[​](#external-cancellation-example "Direct link to External cancellation example")
...
### nonCancellable example[​](#noncancellable-example "Direct link to nonCancellable example")
...
### withTimeout example[​](#withtimeout-example "Direct link to withTimeout example")
...
### scope.cancelRequested[​](#scopecancelrequested "Direct link to scope.cancelRequested")
...
### Cancellation scopes and callbacks[​](#cancellation-scopes-and-callbacks "Direct link to Cancellation scopes and callbacks")
...
### Nesting cancellation scopes[​](#nesting-cancellation-scopes "Direct link to Nesting cancellation scopes")
...
### Sharing promises between scopes[​](#sharing-promises-between-scopes "Direct link to Sharing promises between scopes")
...
* [How to install the Temporal CLI and run a development server](#run-a-development-server)* [How to install a Temporal SDK](#install-a-temporal-sdk)* [Linting and types in TypeScript](#linting-and-types)* [How to connect a Temporal Client to a Temporal Service](#connect-to-a-dev-cluster)* [How to connect to Temporal Cloud](#connect-to-temporal-cloud)* [How to develop a basic Workflow](#develop-workflows)* [How to develop a basic Activity](#develop-activities)* [How to start an Activity Execution](#activity-execution)* [How to run Worker Processes](#run-a-dev-worker)* [How to run a Worker on Docker in TypeScript](#run-a-worker-on-docker)* [How to run a Temporal Cloud Worker](#run-a-temporal-cloud-worker)* [How to shut down a Worker and track its state](#shut-down-a-worker)* [How to start a Workflow Execution](#start-workflow-execution)* [Cancellation scopes in Typescript](#cancellation-scopes)