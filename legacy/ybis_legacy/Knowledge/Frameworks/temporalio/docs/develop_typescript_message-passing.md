Workflow message passing - TypeScript SDK | Temporal Platform Documentation
...
* Validators and Event History:
  ...
  + When a Validator throws an error, the Update is rejected and `WorkflowExecutionUpdateAccepted` *won't* be added to the Event History.
...
#### Use wait conditions in handlers[​](#use-wait-conditions-in-handlers "Direct link to Use wait conditions in handlers")
...
### Use a lock to prevent concurrent handler execution[​](#control-handler-concurrency "Direct link to Use a lock to prevent concurrent handler execution")
...
## Define Signals and Queries statically or dynamically[​](#dynamic-handler "Direct link to Define Signals and Queries statically or dynamically")
...
### Define Signals and Queries statically[​](#static-signals-and-queries "Direct link to Define Signals and Queries statically")
...
### Define Signals and Queries dynamically[​](#dynamic-signals-and-queries "Direct link to Define Signals and Queries dynamically")
...
**Why setHandler and not OTHER\_API?**
...
* [Write message handlers](#writing-message-handlers)
  + [Query handlers](#queries)+ [Signal handlers](#signals)+ [Update handlers and validators](#updates)* [Send messages](#send-messages)
    + [Send a Query](#send-query)+ [Send a Signal](#send-signal)+ [Send a Signal from a Workflow](#send-signal-from-workflow)+ [Signal-With-Start](#signal-with-start)+ [Send an Update](#send-update-from-client)* [Message handler patterns](#message-handler-patterns)
      + [Use async handlers](#async-handlers)+ [Add wait conditions to block](#add-wait-conditions-to-block)+ [Use a lock to prevent concurrent handler execution](#control-handler-concurrency)* [Message handler troubleshooting](#message-handler-troubleshooting)
        + [Problems when sending a Signal](#signal-problems)+ [Problems when sending an Update](#update-problems)+ [Problems when sending a Query](#query-problems)* [Define Signals and Queries statically or dynamically](#dynamic-handler)
          + [Define Signals and Queries statically](#static-signals-and-queries)+ [Define Signals and Queries dynamically](#dynamic-signals-and-queries)