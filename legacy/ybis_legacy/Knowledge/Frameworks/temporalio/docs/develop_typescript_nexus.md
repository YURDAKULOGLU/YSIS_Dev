Temporal Nexus - TypeScript SDK Feature Guide | Temporal Platform Documentation
...
A Nexus Service handler is defined using the `nexus-rpc`'s [`serviceHandler`](https://nexus-rpc.github.io/sdk-typescript/functions/serviceHandler.html) function. 
Nexus Service handlers are typically defined in the same Worker as the underlying Temporal primitives they abstract.
A Service handler must provide Operation handlers for each Operation declared by the Service. 
...
The `@temporalio/nexus` package provides utilities to help create Nexus Operations that interracts with a Temporal namespace:
...
Simple RPC handlers can be implemented as synchronous Nexus Operation handlers, which is defined in TypeScript as a simple async function. 
...
* [Run the Temporal Development Server with Nexus enabled](#run-the-temporal-nexus-development-server)* [Create caller and handler Namespaces](#create-caller-handler-namespaces)* [Create a Nexus Endpoint to route requests from caller to handler](#create-nexus-endpoint)* [Define the Nexus Service contract](#define-nexus-service-contract)* [Develop a Nexus Service handler and Operation handlers](#develop-nexus-service-operation-handlers)
          + [Develop a Synchronous Nexus Operation handler](#develop-a-synchronous-nexus-operation-handler)+ [Develop an Asynchronous Nexus Operation handler to start a Workflow](#develop-an-asynchronous-nexus-operation-handler-to-start-a-workflow)+ [Register your Nexus Service handler in a Worker](#register-your-nexus-service-handler-in-a-worker)* [Develop a caller Workflow that uses the Nexus Service](#develop-caller-workflow-nexus-service)
