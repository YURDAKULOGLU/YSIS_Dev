...
The most important command for grammar development is `tree-sitter generate`, which reads the grammar in structured form and outputs C files that can be compiled into a shared or static library (e.g., using the [`build`](./build.html) command).
...
the section on [`Structuring Rules Well`](../creating-parsers/3-writing-the-grammar.html#structuring-rules-well)
in the user guide.
...
* `src/node-types.json` provides type information about individual syntax nodes; see the section on [`Static Node Types`](../using-parsers/6-static-node-types.html).
