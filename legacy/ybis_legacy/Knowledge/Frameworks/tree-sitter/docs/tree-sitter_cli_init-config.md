...
The [`tree-sitter highlight`](./highlight.html) command takes one or more file paths, and tries to automatically determine,
...
The [Tree-sitter highlighting system](../3-syntax-highlighting.html) works by annotating ranges of source code with logical
...
The [`tree-sitter parse`](./parse.html) command will output a pretty-printed CST when the `-c/--cst` option is used. You can
