...
* `tree-sitter.json` - The main configuration file that determines how `tree-sitter` interacts with the grammar. If missing, the `init` command will prompt the user for the required fields. See [below](./init.html#structure-of-tree-sitterjson) for the full documentation of the structure of this file.
...
* `injection-regex` — A regex pattern that will be tested against a *language name* to determine whether this language
  ...
  Language injection is described in more detail in [the relevant section](../3-syntax-highlighting.html#language-injection).
