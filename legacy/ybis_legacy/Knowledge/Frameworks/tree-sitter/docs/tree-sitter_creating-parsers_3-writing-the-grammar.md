...
is where the `prec` function [described in the previous page](./2-the-grammar-dsl.html) comes into play. By wrapping a rule with `prec`,
...
Adding fields like this allows you to retrieve nodes using the [field APIs](../using-parsers/2-basic-parsing.html#node-field-names).
...
[Query Syntax](../using-parsers/queries/1-syntax.html#supertype-nodes) for more information.
...
2. **Lexical Precedence** — When the precedence functions described [in the previous page](./2-the-grammar-dsl.html) are used *within*
...
If there is an external scanner it may have [an additional impact](./4-external-scanners.html#other-external-scanner-details) over regular tokens
