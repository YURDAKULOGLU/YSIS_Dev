...
It only shows the *named* nodes, as described in [this section](../using-parsers/2-basic-parsing.html#named-vs-anonymous-nodes) of the page on parser usage.

The expected output section can also *optionally* show the [*field names*](../using-parsers/2-basic-parsing.html#node-field-names) associated with each child
...
as-needed whenever you update it by re-running `tree-sitter generate`, or whenever the [external scanner](./4-external-scanners.html)
file is changed.