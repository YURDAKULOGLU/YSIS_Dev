...
from src.agentic.core.workspace import WorkspaceManager
from src.agentic.intelligence.debate_manager import DebateManager
...
class OrchestratorGraph:
    ...
    Powered by Pydantic for robust state management and modular components.
    Feature-complete with Legacy Spec-First and Patch capabilities.
    """
    ...
    async def _spec_node(self, s: TaskState) -> dict[str, Any]:
        """
        Legacy Feature: Spec-First Workflow.
        Generates a detailed specification before planning.
        """
        if not USE_SPEC_FIRST:
            return {}

        log_event(f"Running Spec-First Workflow for {s.task_id}", component="orchestrator_graph")
        
        # Configure Spec Workflow
        ...
        # We need the workspace path
        ws_manager = WorkspaceManager(s.task_id)
        ws_root, docs_dir, artifacts_dir = ws_manager.ensure_directories()

        try:
            result = await spec_workflow.execute(
                task_id=s.task_id,
                goal=s.task_description,
                details=str(s.context),
                workspace_path=ws_root,
                context=s.context
            )
            
            if not result.success and REQUIRE_SPEC:
                return {"phase": "failed", "final_status": "SPEC_FAILED", "error": result.error}
            
            # Enrich context with spec info
            return {
                "context": {**s.context, "spec_verified": True},
                "phase": "spec"
            }
            
        except Exception as e:
            log_event(f"Spec Workflow Error: {e}", component="orchestrator_graph", level="warning")
            if REQUIRE_SPEC:
                return {"phase": "failed", "final_status": "SPEC_ERROR", "error": str(e)}
            return {}

    async def _planner_node(self, s: TaskState) -> dict[str, Any]:
        log_event(f"Analyzing task: {s.task_id}", component="orchestrator_graph")
        
        # 1. Initialize Workspace via Manager
        ws_manager = WorkspaceManager(s.task_id)
        ws_root, docs_dir, artifacts_dir = ws_manager.ensure_directories()
        
        # 2. RAG & Context Enrichment (The "Brain" Upgrade)
        try:
            from src.agentic.tools.local_rag import get_local_rag
            rag = get_local_rag()
            if rag.is_available():
                log_event("Querying RAG for context...", component="orchestrator_graph")
                # Query for similar code and docs
                rag_results = rag.query(s.task_description, n_results=3)
                
                # Format results for context
                rag_context_str = "\n".join([f"- {r['content']} (Source: {r['metadata'].get('source', 'unknown')})" for r in rag_results])
                
                if s.context is None:
                    s.context = {}
                s.context["rag_context"] = rag_context_str
                log_event(f"RAG Context Injected: {len(rag_results)} items", component="orchestrator_graph")
        except Exception as e:
            log_event(f"RAG Enrichment Failed: {e}", component="orchestrator_graph", level="warning")

        # 3. Check for Architectural Debate
        debate_id = await self.debate_manager.maybe_start_debate(
            s.task_id, s.task_description, docs_dir
        )
        if debate_id:
            log_event(f"Architectural Debate Started: {debate_id}", component="orchestrator_graph")
        ...
        if plan_obj:
            ...
            # Persist Plan to Workspace
            ws_manager.write_plan_stub(s.task_description)

        return {
            "plan": plan_obj, 
            "phase": "plan",
            "artifacts_path": str(artifacts_dir) # Update state with real path
        }
    ...
    async def _commit_node(self, s: TaskState) -> dict[str, Any]:
        ...
        # Write Result Artifact using Manager
        ws_manager = WorkspaceManager(s.task_id)
        ws_manager.write_result_stub(
            s.task_description, 
            status="SUCCESS" if success else "FAILED_COMMIT"
        )
    ...
    async def _failed_node(self, s: TaskState) -> dict[str, Any]:
        ...
        # Mark result as failed via Manager
        ws_manager = WorkspaceManager(s.task_id)
        ws_manager.write_result_stub(s.task_description, status="FAILED")
    ...
    def _build_graph(self):
        ...
        builder.add_node("spec", self._spec_node)
        ...
        # Logic for Spec flow
        def _after_spec(s: TaskState) -> str:
            if s.final_status in ["SPEC_FAILED", "SPEC_ERROR"]:
                return "failed"
            return "planner"
