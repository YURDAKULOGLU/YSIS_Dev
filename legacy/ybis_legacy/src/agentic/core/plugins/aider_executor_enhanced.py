...
class AiderExecutorEnhanced(ExecutorProtocol):
    ...
    async def _execute_direct(
        ...
        # 2. Construct the Machine-Only Hyper-Prompt
        prompt = "### SYSTEM: MACHINE_PROTOCOL_v4 ###\n"
        prompt += "ROLE: STATED_DEVELOPER_UNIT\n"
        prompt += "STRICT_RULES:\n"
        prompt += "1. NO_CONVERSATION: Do not output any text before or after the code blocks.\n"
        prompt += "2. NO_EXPLANATIONS: Do not explain what you did.\n"
        prompt += "3. FORMAT_ONLY: Output ONLY the requested file changes in the current edit format.\n"
        prompt += "4. TYPING_MANDATORY: All functions MUST have parameter and return type hints.\n"
        prompt += "5. DOCSTRINGS_MANDATORY: All classes/functions MUST have Google-style docstrings.\n\n"

        if error_history and len(error_history) > 0:
            prompt += "RECOVERY_DATA:\n"
            for i, err in enumerate(error_history):
                prompt += f"ERR_{i+1}: {err}\n"
            prompt += "ACTION: FIX_AND_REWRITE_FULL_FILE\n\n"

        if project_context:
            prompt += "PROJECT_CONTEXT_SUMMARY:\n"
            prompt += project_context + "\n\n"

        prompt += f"MISSION_OBJECTIVE: {plan.objective}\n"
        prompt += "FILES_TO_PROCESS:\n"
        for f in normalized_files:
            prompt += f"- {f}\n"

        prompt += "\nINSTRUCTION: EXECUTE_NOW\n"
