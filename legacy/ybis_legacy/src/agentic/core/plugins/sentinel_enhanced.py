...
class SentinelVerifierEnhanced(VerifierProtocol):
    ...
    async def verify(self, code_result: CodeResult, sandbox_path: str) -> VerificationResult:
        ...
        artifacts_dir = Path(sandbox_path)
        docs_dir = artifacts_dir.parent / "docs"

        # 0. Artifact Verification (Legacy Protocol Check)
        # Ensure critical documentation exists before code approval
        plan_path = docs_dir / "PLAN.md"
        runbook_path = docs_dir / "RUNBOOK.md"
        result_path = artifacts_dir / "RESULT.md"

        if not plan_path.exists() or plan_path.stat().st_size < 10:
             warnings.append("PLAN.md is missing or empty. Documentation gap.")
        
        if not runbook_path.exists() or runbook_path.stat().st_size < 10:
             warnings.append("RUNBOOK.md is missing or empty. Operational gap.")
        
        # We don't block on RESULT.md yet because it might be written AFTER verification,
        # but we warn if it's missing in a final phase.
