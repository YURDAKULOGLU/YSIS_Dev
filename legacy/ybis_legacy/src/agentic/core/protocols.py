...
class TaskPayload(BaseModel):
    """
    Universal payload for task assignment across different agent frameworks.
    Acts as the 'Common Denominator' for YBIS.
    """
    goal: str
    context: dict[str, Any] = Field(default_factory=dict)
    requirements: list[str] = Field(default_factory=list)
    preferred_executor: str | None = None # e.g., 'openhands', 'aider', 'interpreter'
    metadata: dict[str, Any] = Field(default_factory=dict)

class TaskState(BaseModel):
    """The central State object for the entire factory. Supports task chaining and self-evolution."""
    ...
    # Evolution & Advanced Reasoning (v5.0)
    reasoning_analysis: str | None = None # Analysis from Poetiq
    evolution_feedback: str | None = None # Feedback from EvoAgentX
...
class InterAgentMessage(BaseModel):
    ...
    Supports task delegation via payload.
    """
    # Core Identity
    id: str = Field(default_factory=lambda: f"MSG-{datetime.now().strftime('%Y%m%d%H%M%S')}")
    from_agent: str = Field(description="Agent ID of sender")
    to: str | list[str] = Field(description="Recipient agent ID(s) or 'all'")
    ...
    # Embedded Task (Universal Denominator)
