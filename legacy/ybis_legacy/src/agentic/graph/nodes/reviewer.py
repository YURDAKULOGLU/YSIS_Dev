...
async def reviewer_node(state: FactoryState):
    ...
    if not (result.lint_passed and result.tests_passed):
        print("[ERROR] CODE REJECTED (Static Analysis).")
    ...
    if not code_content:
        print("   [WARN] No code found to review.")
    ...
    if keywords:
        ...
        try:
            ...
            print(f"   [WARN] LLM Review Failed: {e}")

    if compliance_issues:
        print("[ERROR] CODE REJECTED (Semantic Compliance).")
        print(compliance_issues[0])
        return _reject(state, compliance_issues)

    print("[OK] CODE APPROVED. Quality Standards Met.")
    return {
        "status": "APPROVED",
