...
# Import Expansion Bridges
from src.agentic.bridges.evo_bridge import EvoAgentBridge
from src.agentic.bridges.swarm_bridge import SwarmBridge
...
# Initialize Bridges
evo_bridge = EvoAgentBridge()
swarm_bridge = SwarmBridge()
poetiq_bridge = PoetiqBridge()

# --- REGISTER TOOLS ---

@mcp.tool()
def evolve_code(source_code: str, feedback: str) -> str:
    """
    Uses EvoAgentX to autonomously evolve and optimize source code.
    Use this to refactor or improve existing system components.
    """
    return evo_bridge.evolve_agent(source_code, feedback)

@mcp.tool()
def deploy_swarm(tool_description: str) -> str:
    """
    Deploys a Self-Improvement Swarm to build a new tool or capability.
    Use this when the system needs a new skill it doesn't currently possess.
    """
    return swarm_bridge.create_tool(tool_description)

@mcp.tool()
def arc_reason(problem: str) -> str:
    """
    Uses Poetiq meta-intelligence for deep reasoning or solving ARC benchmark tasks.
    Use this for high-level architectural decisions or complex logic puzzles.
    """
    import asyncio
    # Poetiq bridge is async, but FastMCP tools are usually sync or async. 
    # Handling as a sync wrapper for simplicity if needed.
    return asyncio.run(poetiq_bridge.reason(problem, {}))

@mcp.tool()
def open_interpreter_exec(code: str) -> str:
    """
    Runs Python code in a stateful terminal environment via Open-Interpreter.
    Use this for direct file manipulation, data analysis, or script execution.
    """
    try:
        from src.agentic.bridges.interpreter_bridge import InterpreterBridge
