...
# Suppress annoying telemetry errors
os.environ["POSTHOG_DISABLED"] = "1"
os.environ["ANONYMIZED_TELEMETRY"] = "False"

# Filter stderr to hide specific library noises
class StderrFilter:
...
def _build_task_context(task_description: str, base_root: Path) -> dict:        
    ...
    context["optional_strategies"] = _detect_optional_strategies()
    return context


def _detect_optional_strategies() -> dict:
    strategies: dict[str, dict] = {}
    try:
        from src.agentic.core.adapters.poetiq_adapter import PoetiqAdapter
        adapter = PoetiqAdapter()
        if adapter.is_available():
            strategies["poetiq_arc_agi"] = {
                "adapter": "PoetiqAdapter",
                "available": True,
                "notes": "ARC-AGI solver adapter under tools/poetiq-arc-agi-solver",
            }
            log_event("Poetiq adapter detected for optional strategy use", component="pipeline")
    except Exception as exc:
        log_event(f"Poetiq adapter unavailable: {exc}", component="pipeline", level="warning")
    return strategies
