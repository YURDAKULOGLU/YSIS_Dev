...
def test_sandbox():
    ...
    try:
        ...
        print(f"✅ Write: Successfully wrote to {abs_path}")
        
        if os.path.exists(abs_path) and not os.path.exists(os.path.join(project_root, test_file)):
            print("✅ Isolation: File exists in sandbox but NOT in real project.")
        else:
            print("❌ Isolation: FAILED! File might have leaked.")
...
def test_ollama():
    ...
    try:
        ...
        print("❌ Ollama Server: OFFLINE (Is 'ollama serve' running?)")
        return

    # 2. Check routing logic
    model_str, decision = router.route(TaskType.CODE_GENERATION, force_local=True)
    print(f"✅ Routing: Selected '{model_str}' ({decision.reason})")
