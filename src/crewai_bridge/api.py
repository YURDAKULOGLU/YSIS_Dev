...
def send_message():
    ...
    try:
        ...
        if not isinstance(data, dict) or 'message' not in data or 'component' not in data:
            error_message = "Geçersiz istek gövdesi. Lütfen 'message' ve 'component' alanlarını içeren bir JSON nesnesi gönderin."
        ...
        logging.error(f"Mesaj işleme sırasında hata oluştu (İstemci IP: {request.remote_addr}): {str(e)}", exc_info=True)
        return create_response("error", f"Dahili sunucu hatası: {str(e)}")
...
def receive_message():
    ...
    try:
        ...
        logging.error(f"Mesaj alma sırasında hata oluştu (İstemci IP: {request.remote_addr}): {str(e)}", exc_info=True)
        return create_response("error", f"Dahili sunucu hatası: {str(e)}")
