...
def check_for_regressions(file, code_complexity, previous_complexity) -> dict:
    ...
    if code_complexity['lines'] > previous_complexity['lines']:
        regression_details['lines'] = f"Satırlar: {code_complexity['lines']} (önceki: {previous_complexity['lines']})"
    if code_complexity['functions'] > previous_complexity['functions']:
        regression_details['functions'] = f"Fonksiyonlar: {code_complexity['functions']} (önceki: {previous_complexity['functions']})"
    if code_complexity['classes'] > previous_complexity['classes']:
        regression_details['classes'] = f"Sınıflar: {code_complexity['classes']} (önceki: {previous_complexity['classes']})"

    if regression_details:
        logging.warning(f"{file} dosyasında gerileme algılandı:\n" + "\n".join([f"  - {detail}" for detail in regression_details.values()]))
