...
async def test_router_integration():
    print("\n[ROUTER TEST] 🧠 Testing Brain Transplant...")
    ...
    if "qwen2.5-coder:32b" not in planning_config.model_name:
        print("[ROUTER TEST] ❌ Unexpected model! Expected 32b beast.")
        return
    ...
    if "qwen2.5-coder:32b" in planner.name():
        print("[ROUTER TEST] ✅ Planner is correctly using the Router!")
    else:
        print(f"[ROUTER TEST] ❌ Planner failed to pickup model. Name is: {planner.name()}")
