...
async def main():
    print("[INFO] INITIALIZING HYBRID SYSTEM (Orchestrator + Aider + Sentinel)...")
    ...
    print(f"[SUCCESS] System Ready: {orchestrator.get_status()}")
    ...
    if result.phase == "done":
        print(f"\n[SUCCESS] SUCCESS! Task completed. Artifacts in {result.artifacts_path}")
    else:
        print(f"\n[ERROR] FAILURE! Task failed with error: {result.error}")
